...
export default function RunGraphView({ graph, onSelectNode, selectedNodeId }: RunGraphViewProps) {
    ...
    <section className="workbench-panel" aria-labelledby="run-graph-title">
      ...
      <ol className="workbench-graph" aria-label="Run sequence">
        {graph.nodes.map((node) => (
          <li key={node.id}>
            <button type="button" className={`workbench-node workbench-node--${node.state}`} onClick={() => onSelectNode?.(node)} aria-label={`${node.kind}: ${node.state}`} aria-pressed={selectedNodeId === node.id}>
