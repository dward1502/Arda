...
export default function RunTimeline({ events, graph, streamStatus = 'idle' }: RunTimelineProps) {
  ...
  const freshness = {
    idle: graph ? 'Unavailable: no live event stream; durable snapshot shown.' : 'Unavailable: no run data loaded.',
    connecting: 'Partial: durable snapshot loaded; live event stream is connecting.',
    live: 'Live: canonical run events are current.',
    error: 'Stale: durable snapshot retained; live event stream is unavailable.',
  }[streamStatus]
  return (
    <section className={`workbench-panel workbench-timeline${reducedMotion ? ' workbench-timeline--reduced-motion' : ''}`} aria-labelledby="run-timeline-title">
      ...
      <dl className="workbench-receipt"><div><dt>Cost</dt><dd>${maxCost.toFixed(2)} maximum</dd></div><div><dt>Resume</dt><dd>{recoveryToken ?? (graph ? `Reload run ${graph.run_id}` : 'No run to resume')}</dd></div><div><dt>Freshness</dt><dd>{freshness}</dd></div></dl>
