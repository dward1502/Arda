...
const CONTRACT: &str = "annunimas.human_ingestion_result.v1";
...
pub fn scan_human_root(
        ...
        if !record.conflicts.is_empty() {
            ...
            if let Some(writer) = contradiction_writer.as_mut() {
                serde_json::to_writer(
                    &mut *writer,
                    &json!({
                        "contract": "annunimas.human_contradiction_candidate.v1",
    ...
    Ok(HumanScanReport {
        schema_version: "annunimas.human-scan-report.v1".to_string(),
...
mod tests {
    ...
    fn scanner_emits_contract_jsonl_without_moving_human_files() {
        ...
        assert_eq!(value["contract"], "annunimas.human_ingestion_result.v1");
