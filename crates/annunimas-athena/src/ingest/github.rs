...
//
// Auth: reads `ANNUNIMAS_GITHUB_TOKEN` (preferred) or `GITHUB_TOKEN` from env.
...
fn offline_forced() -> bool {
    std::env::var("ANNUNIMAS_ATHENA_FORCE_OFFLINE_GITHUB_METADATA")
        .map(|v| v.trim().eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn github_token() -> Option<String> {
    std::env::var("ANNUNIMAS_GITHUB_TOKEN")
        .ok()
