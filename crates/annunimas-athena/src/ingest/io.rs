...
impl AthenaStore {
    ...
    fn knowledge_triage_registry_path(&self) -> Option<PathBuf> {
        if let Ok(path) = std::env::var("ANNUNIMAS_KNOWLEDGE_TRIAGE_REGISTRY_PATH") {
            return Some(PathBuf::from(path));
        }

        let root = super::layout::annunimas_root();
        let canonical_store_root = root.join("data/athena");
        if std::env::var_os("ANNUNIMAS_ROOT").is_none() && self.root != canonical_store_root {
            return None;
        }
    ...
    pub(super) fn emit_ingest_triage_entry(&self, record: &IngestRecord) -> Result<()> {
        ...
        let entry = KnowledgeTriageEntry {
            schema_version: "annunimas.knowledge_triage.v1".to_string(),
            path,
