...
pub(super) fn resolve_inference_route_snapshot(
    ...
    let origin_default = std::env::var("ANNUNIMAS_ROUTE_ORIGIN_DEFAULT")
    ...
    let route_url = std::env::var("ANNUNIMAS_INFERENCE_ROUTER_ROUTE_URL").ok();
    let socket_path = std::env::var("ANNUNIMAS_INFERENCE_ROUTER_SOCKET")
        .or_else(|_| std::env::var("ANNUNIMAS_CHARON_SOCKET"))
        .map(PathBuf::from)
