use super::{CharonService, ProviderState};
...
fn clear_catalog_missing_quarantine_if_live(
    model: &mut crate::types::ModelState,
    live_set: &BTreeSet<String>,
) {
    if model_is_live(&model.id, live_set)
        && model
...
fn catalog_reconciliation_enabled() -> bool {
    std::env::var("ANNUNIMAS_CHARON_CATALOG_RECONCILE_ENABLED")
        .map(|value| !matches!(value.to_ascii_lowercase().as_str(), "0" | "false" | "no"))
        .unwrap_or(true)
}

fn initial_delay_seconds() -> u64 {
    std::env::var("ANNUNIMAS_CHARON_CATALOG_RECONCILE_INITIAL_DELAY_SECONDS")
        .ok()
...
fn interval_seconds() -> u64 {
    std::env::var("ANNUNIMAS_CHARON_CATALOG_RECONCILE_INTERVAL_SECONDS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(21_600)
}
