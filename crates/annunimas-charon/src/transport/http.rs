...
fn http_request_limit() -> usize {
    std::env::var("ANNUNIMAS_CHARON_HTTP_MAX_CONCURRENCY")
        .ok()
...
fn probe_failure_throttle_seconds(provider: &ProviderState) -> i64 {
    let env_key = if provider.access_tier.contains("paid") {
        "ANNUNIMAS_CHARON_PROBE_PAID_FAILURE_THROTTLE_SECONDS"
    } else {
        "ANNUNIMAS_CHARON_PROBE_FAILURE_THROTTLE_SECONDS"
    };
    std::env::var(env_key)
        .ok()
...
fn probe_recent_event_limit() -> usize {
    std::env::var("ANNUNIMAS_CHARON_PROBE_RECENT_EVENT_LIMIT")
        .ok()
...
fn should_emulate_streaming_tool_response(req: &CharonRequestEnvelope) -> bool {
    let enabled = std::env::var("ANNUNIMAS_CHARON_EMULATE_TOOL_STREAMING")
        .ok()
        ...
        .or_else(|| {
            headers.and_then(|headers| {
                header_str(headers, "x-charon-agent-id")
                    .or_else(|| header_str(headers, "x-annunimas-agent-id"))
            })
        })
    ...
    let task_type = headers
        .and_then(|headers| {
            header_str(headers, "x-charon-task-type")
                .or_else(|| header_str(headers, "x-annunimas-task-type"))
        })
        .map(str::to_string)
...
fn apply_openai_route_headers(headers: &HeaderMap, options: &mut Value) {
    ...
    if let Some(value) = header_str(headers, "x-charon-context-window-target")
        .or_else(|| header_str(headers, "x-annunimas-context-window-target"))
    {
...
mod tests {
    ...
    fn streaming_tool_turn_uses_emulation_gate() {
        ...
        std::env::remove_var("ANNUNIMAS_CHARON_EMULATE_TOOL_STREAMING");
        ...
        std::env::set_var("ANNUNIMAS_CHARON_EMULATE_TOOL_STREAMING", "false");
        assert!(!should_emulate_streaming_tool_response(&req));
        std::env::remove_var("ANNUNIMAS_CHARON_EMULATE_TOOL_STREAMING");
    }
    ...
    fn http_request_limit_uses_env_when_valid_and_falls_back_otherwise() {
        ...
        std::env::remove_var("ANNUNIMAS_CHARON_HTTP_MAX_CONCURRENCY");
        assert_eq!(http_request_limit(), 24);

        std::env::set_var("ANNUNIMAS_CHARON_HTTP_MAX_CONCURRENCY", "64");
        assert_eq!(http_request_limit(), 64);

        std::env::set_var("ANNUNIMAS_CHARON_HTTP_MAX_CONCURRENCY", "0");
        assert_eq!(http_request_limit(), 24);

        std::env::set_var("ANNUNIMAS_CHARON_HTTP_MAX_CONCURRENCY", "invalid");
        assert_eq!(http_request_limit(), 24);
...
"#,
        ...
        std::env::set_var("ANNUNIMAS_CHARON_PROVIDER_CONFIG", &config_path);
        let service = CharonService::new(dir.path()).expect("service");
