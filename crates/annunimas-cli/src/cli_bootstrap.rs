use annunimas_athena::transport::expand_home;
use annunimas_athena::AthenaAgent;
use annunimas_core::config::Config;
use annunimas_core::llm::{LlmProvider, OpenAiCompatibleProvider};
...
pub(crate) fn build_router(
    ...
    router.register(Box::new(
        annunimas_hades::HadesAgent::new().context("failed to initialize Hades agent")?,
    ));
    router.register(Box::new(
        annunimas_hermes::HermesAgent::new().context("failed to initialize Hermes agent")?,
    ));
    Ok(router)
}
...
pub(crate) fn set_runtime_defaults() -> anyhow::Result<()> {
    ...
    let homes = [
        ("ANNUNIMAS_ATHENA_HOME", cwd.join("data/athena")),
        ("ANNUNIMAS_PROMETHEUS_HOME", cwd.join("data/prometheus")),
        ("ANNUNIMAS_CHARON_HOME", cwd.join("data/charon")),
        ("ANNUNIMAS_HADES_HOME", cwd.join("data/hades")),
        ("ANNUNIMAS_HERMES_HOME", cwd.join("data/hermes")),
        ("ANNUNIMAS_MNEMOSYNE_HOME", cwd.join("data/mnemosyne")),
        ("ANNUNIMAS_APOLLO_HOME", cwd.join("data/apollo")),
        ("ANNUNIMAS_PLUTUS_HOME", cwd.join("data/plutus")),
        ("ANNUNIMAS_ORACLE_HOME", cwd.join("data/oracle")),
    ];
    ...
    for (socket_key, file) in [
        ("ANNUNIMAS_ATHENA_SOCKET", "athena.sock"),
        ("ANNUNIMAS_PROMETHEUS_SOCKET", "prometheus.sock"),
        ("ANNUNIMAS_CHARON_SOCKET", "charon.sock"),
        ("ANNUNIMAS_HADES_SOCKET", "hades.sock"),
        ("ANNUNIMAS_HERMES_SOCKET", "hermes.sock"),
        ("ANNUNIMAS_MNEMOSYNE_SOCKET", "mnemosyne.sock"),
        ("ANNUNIMAS_APOLLO_SOCKET", "apollo.sock"),
        ("ANNUNIMAS_PLUTUS_SOCKET", "plutus.sock"),
        ("ANNUNIMAS_ORACLE_SOCKET", "oracle.sock"),
    ] {
    ...
    if std::env::var_os("ANNUNIMAS_ILLUVATAR_DISCORD_USER").is_none() {
        std::env::set_var("ANNUNIMAS_ILLUVATAR_DISCORD_USER", "illuvatar");
    }
    if std::env::var_os("ANNUNIMAS_TASK_QUEUE_PATH").is_none() {
        std::env::set_var(
    ...
    if std::env::var_os("ANNUNIMAS_DAILY_QUEUE_PATH").is_none() {
        std::env::set_var(
            "ANNUNIMAS_DAILY_QUEUE_PATH",
            cwd.join("core/queue/queue.jsonl")
                .to_string_lossy()
                .to_string(),
        );
    }
    if std::env::var_os("ANNUNIMAS_WARDEN_QUEUE_PATH").is_none() {
        std::env::set_var(
...
fn runtime_socket_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("ANNUNIMAS_RUNTIME_SOCKET_DIR") {
    ...
    if let Some(dir) = std::env::var_os("XDG_RUNTIME_DIR") {
        let path = PathBuf::from(dir).join("annunimas");
        if std::fs::create_dir_all(&path).is_ok() {
            return path;
        }
    }
    if let Some(uid) = std::env::var_os("UID") {
        let path = PathBuf::from(format!("/run/user/{}/annunimas", uid.to_string_lossy()));
        if std::fs::create_dir_all(&path).is_ok() {
            return path;
        }
    }
