...
/// `ANNUNIMAS_ATHENA_LLM_BASE_URL` or the split loopback host/port fallback;
/// Charon picks the model from its routing matrix when model is `"auto"`.
///
/// Overrides:
/// - `ANNUNIMAS_ATHENA_LLM_BASE_URL`: override base URL
/// - `ANNUNIMAS_ATHENA_LLM_MODEL`: override default model id
/// - `ANNUNIMAS_ATHENA_LLM_API_KEY_ENV`: name of env var to read for bearer auth
/// - `ANNUNIMAS_ATHENA_LLM_USE_CONFIG=1`: opt out of Charon and use the
///   configured default provider from `config/default.toml`
fn build_athena_extraction_provider(
    config: &annunimas_core::config::Config,
) -> Arc<dyn LlmProvider> {
    if std::env::var("ANNUNIMAS_ATHENA_LLM_USE_CONFIG")
        .map(|v| v.trim() == "1" || v.trim().eq_ignore_ascii_case("true"))
        .unwrap_or(false)
    {
        return super::super::cli_bootstrap::build_provider(config);
    }
    let base_url = std::env::var("ANNUNIMAS_ATHENA_LLM_BASE_URL")
        .unwrap_or_else(|_| default_charon_llm_base_url());
    let model = std::env::var("ANNUNIMAS_ATHENA_LLM_MODEL").unwrap_or_else(|_| "auto".to_string());
    let api_key = std::env::var("ANNUNIMAS_ATHENA_LLM_API_KEY_ENV")
        .ok()
...
pub(crate) async fn handle(
    ...
    let default_socket_path =
        socket_path_from_env("ANNUNIMAS_ATHENA_SOCKET", "data/athena/athena.sock");
    match command {
        ...
        AthenaCommands::HumanScan {
            ...
            let report = annunimas_athena::human::scan_human_root(
                &human_root,
                &output,
                Some(&contradictions),
                limit,
        ...
        AthenaCommands::Crawl {
            ...
            let crawl_profile = std::env::var("ANNUNIMAS_ATHENA_CRAWL_PROFILE")
                .unwrap_or_else(|_| "production".to_string());
            let provider_order = resolve_crawl_provider_order(
                std::env::var("ANNUNIMAS_ATHENA_CRAWL_PROVIDER")
                    .ok()
                    .as_deref(),
                Some(&crawl_profile),
            );
            let crawl_service =
                std::env::var("ANNUNIMAS_CRAWL4AI_URL").unwrap_or_else(|_| default_crawl4ai_url());
            let mut crawl = None;
...
fn build_packet_intake_surface(
    ...
    for (idx, row) in rows.iter().enumerate() {
        ...
        intake_rows.push(json!({
            "contract": "annunimas.athena.packet_intake.row.v1",
    ...
    Ok(json!({
        "contract": "annunimas.athena.packet_intake.v1",
        "authority": "agent_generated",
        "review_required": true,
...
fn build_packet_promotion_surface(
    ...
    md.push_str("contract: annunimas.athena.packet_promotion_surface.v1\n---\n\n");
    ...
    Ok(json!({
        "contract": "annunimas.athena.packet_promotion_surface.v1",
        "authority": "agent_generated",
        "review_required": true,
        "generated_at_utc": now_utc(),
        "packet_dir": repo_display_path(root, &packet_dir),
...
fn ingest_human_corpus_wave(store: &AthenaStore, limit: usize) -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.human-corpus-wave.v1",
        "generated_at_utc": now_utc(),
...
fn ingest_human_document_wave(store: &AthenaStore, limit: usize) -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.human-document-wave.v1",
        "generated_at_utc": now_utc(),
...
Include at most {limit} results. Do not use xurl or the X API; use only the x_search tool."
    ...
    let capture_root = annunimas_root().join("data/athena/x_search");
