...
fn build_charon_telemetry_report(
    ...
    let record = json!({
        "contract": "annunimas.charon.telemetry_summary.v1",
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("annunimas-cli-{name}-{unique}"));
    ...
    fn charon_telemetry_report_dry_run_is_review_gated_without_writing() {
        ...
        assert_eq!(report["contract"], "annunimas.charon.telemetry_summary.v1");
