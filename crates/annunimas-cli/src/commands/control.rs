...
fn approve_human_augmentation(
    ...
    approvals_state["schema_version"] = json!("annunimas.human-augmentation-approval.v1");
...
fn record_ceo_council_session(
    ...
    sessions_state["schema_version"] = json!("annunimas.ceo-council-sessions.v1");
...
fn runtime_dir() -> PathBuf {
    if let Ok(path) = std::env::var("ANNUNIMAS_RUNTIME_SOCKET_DIR") {
        let p = PathBuf::from(path);
        if p.ends_with("annunimas") {
            return p;
        }
        return p.join("annunimas");
    }
    ...
    PathBuf::from(format!("/run/user/{}/annunimas", nixless_uid()))
}
...
fn prune_runtime_build_cache(out_path: &str) -> anyhow::Result<Value> {
    let root = workspace_root();
    let build_root = std::env::var("ANNUNIMAS_BUILD_CACHE_ROOT")
        .or_else(|_| std::env::var("ANNUNIMAS_RUNTIME_BUILD_ROOT"))
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp/annunimas-build"));
    ...
    let max_age_hours = std::env::var("ANNUNIMAS_BUILD_CACHE_MAX_AGE_HOURS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(24u64);
    let max_bytes = std::env::var("ANNUNIMAS_BUILD_CACHE_MAX_BYTES")
        .ok()
        .and_then(|value| value.parse().ok())
    ...
    let payload = json!({
        "schema_version": "annunimas.runtime-build-cache.v1",
        "generated_at_utc": now_utc_seconds(),
...
fn organize_hades_backups(hades_root: &str, out_path: &str) -> anyhow::Result<Value> {
    ...
    let archive_root = std::env::var("ANNUNIMAS_HADES_ARCHIVE_ROOT")
        .map(PathBuf::from)
    ...
    let payload = json!({
        "schema_version": "annunimas.hades-charon-layout.v1",
        "generated_at_utc": now_utc_seconds(),
...
fn apply_opencode_route_governor(
    ...
    let payload = json!({
        "schema_version": "annunimas.opencode-route-governor.v1",
        "generated_at_utc": now_utc(),
...
fn apply_runtime_recovery_route_governor(
    ...
    let payload = json!({
        "schema_version": "annunimas.runtime-recovery-route-governor.v1",
        "generated_at_utc": now_utc(),
...
fn command_timeout_seconds(override_value: Option<u64>) -> u64 {
    override_value
        .or_else(|| {
            std::env::var("ANNUNIMAS_RUNTIME_RECOVERY_CMD_TIMEOUT_SECONDS")
                .ok()
                .and_then(|value| value.parse().ok())
        })
        .unwrap_or(20)
...
fn command_env() -> std::collections::BTreeMap<String, String> {
    ...
    merged
        .entry("ANNUNIMAS_WORKSPACE_ROOT".to_string())
...
fn run_runtime_admission_recovery_executor(
    ...
    let payload = json!({
        "schema_version": "annunimas.runtime-admission-recovery-executor.v1",
        "generated_at_utc": now_utc(),
...
fn sync_operator_profile(
    ...
    let mut runtime = vec![
        (
            "ANNUNIMAS_TASK_QUEUE_PATH".to_string(),
            paths
                .get("task_queue")
                .and_then(Value::as_str)
        ...
        (
            "ANNUNIMAS_PROJECT_TASK_QUEUE_PATH".to_string(),
            paths
        ...
        (
            "ANNUNIMAS_DAILY_QUEUE_PATH".to_string(),
            paths
                .get("daily_queue")
                .and_then(Value::as_str)
        ...
        (
            "ANNUNIMAS_WARDEN_QUEUE_PATH".to_string(),
            paths
                .get("warden_queue")
                .and_then(Value::as_str)
        ...
        (
            "ANNUNIMAS_HADES_WATCH_PATHS".to_string(),
            hades_watch_paths
                .iter()
                .filter_map(Value::as_str)
                .collect::<Vec<_>>()
        ...
        (
            "ANNUNIMAS_ROUTE_PRIVACY_DEFAULT".to_string(),
            routing
                .get("privacy_default")
                .and_then(Value::as_str)
                .unwrap_or("public")
        ...
        (
            "ANNUNIMAS_ROUTE_COST_DEFAULT".to_string(),
            routing
                .get("cost_default")
                .and_then(Value::as_str)
                .unwrap_or("balanced")
        ...
        (
            "ANNUNIMAS_ROUTE_QUALITY_DEFAULT".to_string(),
            routing
                .get("quality_default")
                .and_then(Value::as_str)
                .unwrap_or("balanced")
        ...
        (
            "ANNUNIMAS_ROUTE_ORIGIN_DEFAULT".to_string(),
            routing
                .get("origin_default")
                .and_then(Value::as_str)
                .unwrap_or("auto")
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_HADES_QUEUE".to_string(),
            retention
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_HADES_LOG".to_string(),
            retention
                .get("hades_log_days")
                .and_then(Value::as_i64)
                .unwrap_or(30)
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_HADES_JOULEWORK".to_string(),
            retention
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_HERMES_MESSAGES".to_string(),
            retention
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_HERMES_QUEUE".to_string(),
            retention
        ...
        (
            "ANNUNIMAS_RETENTION_DAYS_MNEMOSYNE_COMPACT".to_string(),
            retention
        ...
        (
            "ANNUNIMAS_PRESSURE_MAX_TOTAL_KNOWN_WORK".to_string(),
            pressure
                .get("max_total_known_work")
                .and_then(Value::as_i64)
                .unwrap_or(120)
        ...
        (
            "ANNUNIMAS_PRESSURE_MAX_PROJECTS_QUEUE_QUEUED".to_string(),
            pressure
                .get("max_projects_queue_queued")
                .and_then(Value::as_i64)
                .unwrap_or(20)
        ...
        (
            "ANNUNIMAS_PRESSURE_MAX_OVERSIZE_COUNT".to_string(),
            pressure
                .get("max_oversize_count")
                .and_then(Value::as_i64)
                .unwrap_or(0)
        ...
        (
            "ANNUNIMAS_PRESSURE_MAX_DISK_USED_PCT".to_string(),
            pressure
                .get("max_disk_used_pct")
                .and_then(Value::as_i64)
                .unwrap_or(92)
        ...
        (
            "ANNUNIMAS_PRESSURE_MAX_INVALID_JSON".to_string(),
            pressure
                .get("max_invalid_json")
                .and_then(Value::as_i64)
                .unwrap_or(0)
        ...
        (
            "ANNUNIMAS_PRESSURE_REQUIRE_COVERAGE_OK".to_string(),
            pressure
        ...
        (
            "ANNUNIMAS_HANDOFF_LOOKBACK_HOURS".to_string(),
            handoff
                .get("lookback_hours")
                .and_then(Value::as_i64)
                .unwrap_or(24)
        ...
        (
            "ANNUNIMAS_HANDOFF_MAX_P95_MS".to_string(),
            handoff
                .get("max_p95_ms")
                .and_then(Value::as_i64)
                .unwrap_or(2500)
        ...
        (
            "ANNUNIMAS_HANDOFF_MIN_SUCCESS_RATE".to_string(),
            handoff
        ...
        (
            "ANNUNIMAS_HANDOFF_MAX_DEFER_RATE".to_string(),
            handoff
                .get("max_defer_rate")
                .and_then(Value::as_f64)
                .unwrap_or(0.2)
        ...
        (
            "ANNUNIMAS_HANDOFF_MAX_DUPLICATE_RATE".to_string(),
            handoff
        ...
        (
            "ANNUNIMAS_HANDOFF_MAX_STUCK_INTENTS".to_string(),
            handoff
        ...
        (
            "ANNUNIMAS_HANDOFF_STUCK_MINUTES".to_string(),
            handoff
                .get("stuck_minutes")
                .and_then(Value::as_i64)
                .unwrap_or(30)
        ...
        (
            "ANNUNIMAS_AUTONOMY_MIN_ACK_SUCCESS_RATE".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_MAX_P95_LATENCY_MS".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_MAX_QUEUE_STALE_MINUTES".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_MIN_SAFETY_SCORE".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_REQUIRE_ATHENA_LOOKUP_BEFORE_ACTION".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_MAX_ATHENA_LOOKUP_AGE_MINUTES".to_string(),
            autonomy
        ...
        (
            "ANNUNIMAS_AUTONOMY_REQUIRED_LIVE_SOCKETS".to_string(),
        ...
        (
            "ANNUNIMAS_AUTONOMY_MAX_MISSING_REQUIRED_SOCKETS".to_string(),
            autonomy
    ...
    if let Some(latency) = routing.get("latency_sla_ms").and_then(Value::as_i64) {
        runtime.push((
            "ANNUNIMAS_ROUTE_LATENCY_SLA_MS".to_string(),
            latency.to_string(),
        ));
    }
...
fn launch_preflight(
    ...
    let payload = json!({
        ...
        "schema_version": "annunimas.launch-preflight.v1",
        "status": status,
        "lane": lane,
