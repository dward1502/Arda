...
async fn autopilot_handle(command: AutopilotCommands) -> anyhow::Result<()> {
    ...
    match command {
        ...
        AutopilotCommands::KnowledgeTriage {
            ...
            let mut cfg =
                annunimas_prometheus::autopilot::KnowledgeTriageConfig::for_root(&resolved_root)
            ...
            let report = annunimas_prometheus::autopilot::run_knowledge_triage(&cfg)?;
            println!("{}", serde_json::to_string_pretty(&report)?);
        }
        AutopilotCommands::PromoteKnowledgeTasks {
            ...
            let report = annunimas_prometheus::autopilot::promote_knowledge_tasks(&cfg)?;
            println!("{}", serde_json::to_string_pretty(&report)?);
        }
        AutopilotCommands::ExecuteKnowledgeTasks {
            ...
            let cfg =
                annunimas_prometheus::autopilot::KnowledgeTriageConfig::for_root(&resolved_root)
                    .with_dry_run(dry_run || !write);
            let report = annunimas_prometheus::autopilot::execute_knowledge_task_queue(&cfg)?;
            println!("{}", serde_json::to_string_pretty(&report)?);
        }
    }
    Ok(())
