...
fn permission_scope_refresh(
    ...
    let receipt = json!({
        "contract": "annunimas.permission_scope_refresh.v1",
        "profile": profile,
        "scope": scope,
        "ttl_hours": ttl_hours,
        "reason": reason,
...
fn professionalization_audit_closeout(audit_dir: &Path) -> anyhow::Result<Value> {
    ...
    Ok(json!({
        "schema_version": "annunimas.professionalization-audit-closeout.v1",
...
fn merge_bridge_node(node_id: &str, targets: &BTreeMap<String, Value>, bridge: &Value) -> Value {
    ...
    if !merged.contains_key("ssh_user") {
        let ssh_user = target
            ...
            .unwrap_or_else(|| "annunimas".to_string());
...
fn hermes_agent_gateway_receipt(
    ...
    let risk = if normalized_status == "completed" && blockers.is_empty() {
        annunimas_hermes::types::CommsEventRisk::Low
    } else {
        annunimas_hermes::types::CommsEventRisk::Medium
    };
    ...
    Ok(json!({
        "schema_version": "annunimas.hermes_agent_gateway_receipt_adapter.v1",
        "mode": "write",
...
fn agent_conversation_append(
    ...
    if dry_run {
        return Ok(json!({
            "schema_version": "annunimas.council.agent_conversation_append.v1",
            "mode": "dry_run",
            "record_path": path,
            "record": record,
...
fn write_agent_conversation_ledger_and_projection(
    ...
    Ok(json!({
        "schema_version": "annunimas.council.agent_conversation_projection_write.v1",
...
fn scout_request_append(
    ...
    Ok(json!({
        "schema_version": "annunimas.athena.scout_request_append.v1",
...
fn build_scout_request_record(
    ...
    Ok(json!({
        "schema_version": "annunimas.athena.scout_request.v1",
...
fn scout_finding_append(
    ...
    Ok(json!({
        "schema_version": "annunimas.athena.scout_finding_append.v1",
        "mode": if dry_run { "dry_run" } else { "write" },
        "record_path": path,
        "runtime_path": root.join("core/state/scout_runtime.json"),
        "record": record,
...
fn build_scout_finding_record(
    ...
    Ok(json!({
        "schema_version": "annunimas.athena.scout_finding.v1",
...
fn scout_runtime_refresh() -> anyhow::Result<Value> {
    ...
    Ok(json!({
        "schema_version": "annunimas.athena.scout_runtime_refresh.v1",
        "mode": "write",
        "runtime_path": path,
        "runtime_projection": runtime,
        "side_effect_policy": local_ledger_side_effect_policy()
...
fn write_remote_confidence_producer_proof(root: &Path) -> anyhow::Result<Value> {
    ...
    let report = json!({
        "schema_version": "annunimas.remote_confidence_producer_proof.v1",
...
fn build_scout_runtime_projection_with_record(
    ...
    let stale_count = requests
        ...
        + findings
            ...
            .count();
    json!({
        "schema_version": "annunimas.athena.scout_runtime.v1",
        "generated_at_utc": now_utc(),
        "source_ledgers": {
...
fn publish_remote_confidence_snapshot(
    ...
    if let Some(object) = snapshot.as_object_mut() {
        ...
        object.insert(
            "publisher".to_string(),
            json!({
                "schema_version": "annunimas.remote_confidence_publisher.v1",
...
fn build_hermes_agent_gateway_activation_check(
    ...
    let template_path = root.join("config/hermes_agent_gateway_annunimas.example.yaml");
    let semantic_channel_path =
        root.join("crates/annunimas-hermes/src/service/semantic_channel.rs");
    let serenity_bot_path = root.join("crates/annunimas-hermes/src/serenity_bot.rs");
    let annunimas_env_path = root.join("config/.env");
    ...
    let annunimas_env = read_env_file(&annunimas_env_path);
    ...
    let discord_gateway_surface_ready = serenity_source.contains("CreateCommand::new(\"gateway\")")
        ...
        && serenity_source.contains("annunimas.hermes_agent_gateway_background_result.v1")
    ...
    .all(|command| serenity_source.contains(&format!("CreateCommand::new(\"{command}\")")))
        && serenity_source.contains("annunimas.hermes.work_stream_continuation_request.v1")
        && serenity_source.contains("annunimas.hermes.discord_operating_room_interaction.v1")
    ...
    json!({
        "schema_version": "annunimas.hermes_agent_gateway_activation_check.v1",
        "mode": "read_only",
        "checked_at_utc": now_utc(),
        ...
        "artifacts": {
            ...
            "annunimas_env": annunimas_env_path,
            ...
            "annunimas_env_exists": annunimas_env_path.exists(),
        ...
        "live_mapping_candidates": {
            "exact_annunimas_vars_missing": exact_env_missing,
...
fn first_present_env(
    ...
    annunimas_env: &BTreeMap<String, String>,
    ...
    for key in keys {
        ...
        if let Some(value) = annunimas_env
            .get(*key)
...
fn default_charon_control_url() -> String {
    std::env::var("ANNUNIMAS_CHARON_CONTROL_URL")
...
fn create_crate_spawn_blueprint(
    ...
    let core_path = pathdiff::diff_paths(root.join("crates/annunimas-core"), &crate_root)
        .unwrap_or_else(|| PathBuf::from("../../annunimas-core"));
    let governance_path =
        pathdiff::diff_paths(root.join("crates/annunimas-governance"), &crate_root)
            .unwrap_or_else(|| PathBuf::from("../../annunimas-governance"));
    ...
    write_file(
        ...
        format!(
            "# {crate_name}\n\nSpawned from the Annunimas sovereign crate blueprint.\n\n- Realm: `{realm}`\n- Productizable: `{}`\n- Required exports: `core/state/{crate_name}.json`\n",
    ...
    Ok(json!({
        "schema_version": "annunimas.crate-spawn-receipt.v1",
        "generated_at_utc": now_utc(),
        "crate_name": crate_name,
        "realm": realm,
        "productizable": productizable,
...
fn repair_hades_stores(apply: bool, report_path: &str) -> anyhow::Result<Value> {
    ...
    let plan = [
        (
            ...
            "hades_log",
            std::env::var("ANNUNIMAS_HADES_LOG_MAX_KEEP")
                .ok()
        ...
        (
            ...
            "warden_queue",
            std::env::var("ANNUNIMAS_HADES_WARDEN_QUEUE_MAX_KEEP")
                .ok()
                .and_then(|v| v.parse().ok())
        ...
        (
            ...
            "action_queue",
            std::env::var("ANNUNIMAS_HADES_QUEUE_MAX_KEEP")
                .ok()
                .and_then(|v| v.parse().ok())
...
fn candidate_cli_bins() -> Vec<PathBuf> {
    let build_root = PathBuf::from(
        std::env::var("ANNUNIMAS_BUILD_CACHE_ROOT")
            .or_else(|_| std::env::var("ANNUNIMAS_RUNTIME_BUILD_ROOT"))
            .unwrap_or_else(|_| "/tmp/annunimas-build".to_string()),
    );
    vec![
        build_root.join("target/debug/annunimas-cli"),
        workspace_root().join("target/debug/annunimas-cli"),
    ]
}
...
mod tests {
    ...
    fn unique_test_audit_dir(name: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("annunimas-{name}-{nanos}"))
    }
    ...
    fn seed_audit_closeout_fixture() -> PathBuf {
        ...
        write_jsonl(
            ...
            &[
                ...
                json!({"id":"P8-HARD-004","status":"queued","title":"Add packaged audit closeout command","owner":"annunimas-cli","scope":"annunimas-cli audit/status surface"}),
    ...
    fn professionalization_audit_closeout_reports_closeout_and_hardening_state() {
        ...
        assert_eq!(
            ...
            "annunimas.professionalization-audit-closeout.v1"
        );
    ...
    #[test]
    fn hermes_agent_gateway_receipt_keeps_annunimas_policy_boundary() {
        let receipt = build_hermes_agent_gateway_receipt(
            ...
            &["cargo test -p annunimas-hermes".to_string()],
            &["crates/annunimas-hermes/src/serenity_bot.rs".to_string()],
            &[],
            None,
        );
        ...
        assert_eq!(
            ...
            "annunimas.hermes_agent_gateway_background_result.v1"
        );
        ...
        assert_eq!(
            receipt["policy_boundary"]["annunimas_records_authority"],
            true
        );
        assert_eq!(
    ...
    fn hermes_agent_gateway_activation_check_reports_human_gated_live_state() {
        ...
        fs::create_dir_all(dir.join("crates/annunimas-hermes/src/service"))
            .expect("create semantic channel dir");
        fs::create_dir_all(dir.join("crates/annunimas-hermes/src")).expect("create hermes src dir");
        fs::write(
        ...
        fs::write(
            dir.join("config/hermes_agent_gateway_annunimas.example.yaml"),
            "allow_from:\nallow_admin_from:\nbackground_process_notifications: result\nhermes-agent-gateway-receipt\n",
        )
        .expect("write template");
        fs::write(
            dir.join("crates/annunimas-hermes/src/service/semantic_channel.rs"),
            r#""work-stream" "workstream" "work" "tasks""#,
        )
        .expect("write semantic channel source");
        fs::write(
            dir.join("crates/annunimas-hermes/src/serenity_bot.rs"),
            r#"CreateCommand::new("plans") CreateCommand::new("tasks") CreateCommand::new("task") CreateCommand::new("review") CreateCommand::new("continue") CreateCommand::new("council") CreateCommand::new("gateway") "activation_check" "remote_confidence" "record_receipt" annunimas.hermes_agent_gateway_background_result.v1 gateway_result_is_not_approval annunimas.hermes.work_stream_continuation_request.v1 annunimas.hermes.discord_operating_room_interaction.v1"#,
        )
        ...
        let env = BTreeMap::from([
            (
                "ANNUNIMAS_DISCORD_WORK_STREAM_CHANNEL_ID".to_string(),
                "missing".to_string(),
            ),
            (
                "ANNUNIMAS_OPERATOR_DISCORD_USER_ID".to_string(),
                "missing".to_string(),
            ),
        ...
        assert_eq!(
            ...
            "annunimas.hermes_agent_gateway_activation_check.v1"
        );
        ...
        assert_eq!(
            check["annunimas_hermes"]["receipt_adapter"]["default_mode"],
            "dry_run"
        );
        assert_eq!(
            check["annunimas_hermes"]["discord_gateway_surface"]["ready"],
            true
        );
        assert_eq!(
            check["annunimas_hermes"]["operating_room_surface"]["ready"],
            true
        );
        assert_eq!(
    ...
    fn remote_confidence_snapshot_is_read_only_and_summarizes_attention_gates() {
        ...
        assert_eq!(
            ...
            "annunimas.remote_confidence_snapshot.v1"
        );
    ...
    fn remote_confidence_publisher_writes_only_local_arda_state_snapshot() {
        ...
        assert_eq!(
            ...
            "annunimas.remote_confidence_publisher.v1"
        );
    ...
    fn safe_local_work_cycle_preflight_writes_only_report_and_does_not_mutate_queue() {
        ...
        assert_eq!(
            receipt["schema_version"],
            "annunimas.safe_local_work_cycle_preflight.v1"
        );
    ...
    fn conversation_and_scout_ledgers_are_local_only_and_projectable() {
        ...
        assert_eq!(
            ...
            "annunimas.athena.scout_request.v1"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "annunimas.athena.scout_finding.v1"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "annunimas.athena.scout_runtime.v1"
        );
    ...
    fn scout_runtime_projection_exposes_conversation_evidence_without_external_side_effects() {
        ...
        assert_eq!(
            report["schema_version"],
            "annunimas.council.agent_conversation_projection_write.v1"
        );
    ...
    fn remote_confidence_producer_proof_wires_conversation_and_scout_ledgers() {
        ...
        assert_eq!(
            report["schema_version"],
            "annunimas.remote_confidence_producer_proof.v1"
        );
    ...
    fn bridge_alias_target_id_inherits_current_edge_target_connection_fields() {
        ...
        let bridge = json!({
            ...
            "node": {
                "node-backbone-server": {
                    ...
                    "remote_cwd": "/var/home/annunimasserver/.hermes/hermes-agent"
                }
            }
        });
        ...
        assert_eq!(node["ssh_user"], "annunimasserver");
