...
pub(crate) fn export_embodied_interface_impl() -> Result<Value> {
    ...
    let realm_data = read_toml_or(
        &root.join("core/realm/annunimas.toml"),
    ...
    let payload = json!({
        "schema_version": "annunimas.embodied-interface.v1",
        ...
        "rendering_rules": {
            ...
            "color_source": "core/realm/annunimas.toml realm colors",
...
pub(crate) fn export_tauri_embodiment_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.tauri-embodiment.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_legion_hierarchy_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.legion-hierarchy.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_task_agent_boundaries_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.task-agent-boundaries.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_soterion_joulework_enforcement_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.soterion-joulework-enforcement.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_imported_capability_reconciliation_impl() -> Result<Value> {
    ...
    let athena_payload = json!({
        "schema_version": "annunimas.athena-imported-capability-roadmap.v1",
        "generated_at_utc": now_utc(),
    ...
    let hermes_payload = json!({
        "schema_version": "annunimas.hermes-imported-capability-roadmap.v1",
        "generated_at_utc": now_utc(),
        "authority": "rank2_capability_reconciliation",
        "status": "roadmap_ready_for_execution_review",
    ...
    let memo_payload = json!({
        "schema_version": "annunimas.imported-tool-fit-decision-memo.v1",
        "generated_at_utc": now_utc(),
        "authority": "rank2_capability_reconciliation",
