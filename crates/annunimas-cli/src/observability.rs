...
pub(crate) fn build_operations_briefing(
    ...
    if core_root.join("realm/boot.toml").exists() {
        let _ = annunimas_prometheus::CoreAutonomyProfile::load(&core_root);
    }
...
pub(crate) fn build_operations_briefing_from_runtime_inputs(
    ...
    json!({
        "contract": "annunimas.operations_briefing.v1",
...
pub(crate) fn format_operations_briefing_text(briefing: &serde_json::Value) -> String {
    ...
    let mut lines = vec![
        "Annunimas Operations Briefing".to_string(),
        format!(
            "Latest cycle: {}",
            latest
                .get("timestamp")
...
pub(crate) async fn build_ops_dashboard(core_root: &str) -> anyhow::Result<serde_json::Value> {
    if std::env::var("ANNUNIMAS_OPS_DASHBOARD_LIVE")
        .ok()
        .as_deref()
        != Some("1")
    {
        ...
        return Ok(json!({
            ...
            "live_service_collection_hint": "set ANNUNIMAS_OPS_DASHBOARD_LIVE=1 for the legacy live service sweep",
...
fn hades_action_queue_observability(
    ...
    let closed_task_ids = closeout_records
        ...
        .filter(|record| {
            record.get("contract").and_then(|value| value.as_str())
                == Some("annunimas.hades.action_queue_closeout.v1")
    ...
    HadesActionQueueObservability {
        ...
        closeout_contract: "annunimas.hades.action_queue_closeout.v1".to_string(),
    }
}
...
pub(crate) fn build_governance_observation(
    ...
    let thresholds = system_control
        ...
        .cloned()
        .unwrap_or_else(|| default_signal_thresholds("annunimas_totality"));
    let weights = system_control
        .get("governance")
        .and_then(|v| v.get("weights"))
        .cloned()
        .unwrap_or_else(|| default_governance_weights("annunimas_totality"));
    ...
    let mut observation = json!({
        ...
        "goal": {
            ...
            "active_ruleset": ruleset.get("active_ruleset").cloned().unwrap_or(serde_json::json!("annunimas_totality"))
        }
    });
...
mod tests {
    ...
    fn operations_briefing_surfaces_compact_periodic_summary_contract() {
        let briefing = build_operations_briefing_from_inputs(
            &serde_json::json!({
                ...
                "services": {
                    ...
                    "services": [
                        {"unit": "annunimas-ceo-autopilot-supervised.service", "active": "activating", "sub": "start", "note": "activating (start)", "score": 0.6},
                        {"unit": "annunimas-charon.service", "active": "active", "sub": "running", "note": "running", "score": 1.0}
                    ]
                },
                "dashboard": {
        ...
        assert_eq!(briefing["contract"], "annunimas.operations_briefing.v1");
    ...
    fn operations_briefing_text_readout_is_operator_facing_and_non_mutating() {
        ...
        assert!(readout.contains("Annunimas Operations Briefing"));
        ...
        assert!(readout
            .contains("- annunimas-ceo-autopilot-supervised.service: activating/start score=0.6"));
    ...
    fn hades_action_queue_folds_append_only_closeout_ledger() -> anyhow::Result<()> {
        ...
        fs::write(
            ...
            format!(
                "{{\"contract\":\"annunimas.hades.action_queue_closeout.v1\",\"source_queue\":{source_queue_json},\"task_id\":\"hds_closed\",\"source_queue_mutated\":false,\"status\":\"completed_remediated\"}}\n{{\"contract\":\"annunimas.hades.action_queue_closeout.v1\",\"source_queue\":{source_queue_json},\"task_id\":\"hds_unrelated\",\"source_queue_mutated\":false,\"status\":\"completed_remediated\"}}\n",
        ...
        assert_eq!(
            ...
            "annunimas.hades.action_queue_closeout.v1"
        );
