...
impl Config {
    pub fn load(path: impl AsRef<Path>) -> Result<Self> {
        let content = std::fs::read_to_string(path.as_ref()).map_err(|e| {
            crate::error::AnnunimasError::Config(format!("Failed to read config: {}", e))
        })?;
        toml::from_str(&content).map_err(|e| {
            crate::error::AnnunimasError::Config(format!("Failed to parse config: {}", e))
        })
    }
}
