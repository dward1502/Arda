...
Scope: `crates/annunimas-governance/` — 1,094 LOC across 8 modules (`triad.rs`, `bacon_lite.rs`, `resonance.rs`, `joulework.rs`, `love_equation.rs`, `game_theory.rs`, `solar.rs`, `lib.rs`). Pure deterministic decision logic — no LLM calls, no async, only one I/O writer (bacon-lite ledger).
...
- Touch: new `bacon_lite::summary` fn, `annunimas-cli` reader command.
