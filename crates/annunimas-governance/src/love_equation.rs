...
pub fn love_equation_score(task: &Task) -> LoveEquationScore {
    let impact = match task.status {
        annunimas_core::TaskStatus::Complete => 0.95,
        annunimas_core::TaskStatus::Running => 0.7,
        annunimas_core::TaskStatus::Pending => 0.55,
        annunimas_core::TaskStatus::Retry { .. } => 0.45,
        annunimas_core::TaskStatus::Failed { .. } => 0.2,
    };
