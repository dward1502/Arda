...
//
// Run:  cargo bench -p annunimas-hades
...
fn fresh_service() -> (tempfile::TempDir, annunimas_hades::HadesService) {
    ...
    let svc = annunimas_hades::HadesService::new(dir.path()).expect("hades service");
    (dir, svc)
}
...
fn populate_queue(dir: &Path, n: usize) {
    use annunimas_hades::{ActionKind, QuorumProof, TaskItem};
...
fn write_world_state(hades_dir: &Path, n_references: usize) {
    ...
    // ANNUNIMAS_WORLD_STATE_PATH. Write a file with many entries.
    ...
    std::env::set_var("ANNUNIMAS_WORLD_STATE_PATH", &world_path);
}
