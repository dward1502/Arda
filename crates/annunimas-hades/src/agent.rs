...
use annunimas_core::agent::{Agent, AgentManifest};
use annunimas_core::error::{AnnunimasError, Result};
use annunimas_core::task::Task;
use annunimas_core::SoterionMeta;
...
impl HadesAgent {
    pub fn new() -> Result<Self> {
        ...
        let service =
            HadesService::from_default_or_fallback().map_err(|err| AnnunimasError::Agent {
...
mod tests {
    ...
    fn initializes_hades_agent_from_runtime_home() {
        ...
        let original = std::env::var_os("ANNUNIMAS_HADES_HOME");
        std::env::set_var("ANNUNIMAS_HADES_HOME", dir.path());
        ...
        match original {
            Some(value) => std::env::set_var("ANNUNIMAS_HADES_HOME", value),
            None => std::env::remove_var("ANNUNIMAS_HADES_HOME"),
        }
    }
}
