...
pub(super) fn default_root() -> PathBuf {
    if let Ok(custom) = std::env::var("ANNUNIMAS_HADES_HOME") {
...
pub(super) fn default_destructive_policy_path() -> PathBuf {
    if let Ok(custom) = std::env::var("ANNUNIMAS_HADES_DESTRUCTIVE_POLICY_PATH") {
        return PathBuf::from(custom);
    }
...
pub(super) fn default_watch_paths() -> Vec<PathBuf> {
    if let Ok(custom) = std::env::var("ANNUNIMAS_HADES_WATCH_PATHS") {
        let parsed = custom
            .split(':')
...
pub(super) fn sweep_interval_hours() -> i64 {
    std::env::var("ANNUNIMAS_HADES_SWEEP_INTERVAL_HOURS")
        .ok()
...
pub(super) fn scheduler_snapshot() -> serde_json::Value {
    let morning_hour_utc = std::env::var("ANNUNIMAS_HADES_MORNING_HOUR_UTC")
        .ok()
        .and_then(|v| v.parse::<u8>().ok())
        .map(|v| v.min(23))
        .unwrap_or(14);
    let nightly_hour_utc = std::env::var("ANNUNIMAS_HADES_NIGHTLY_HOUR_UTC")
        .ok()
        .and_then(|v| v.parse::<u8>().ok())
        .map(|v| v.min(23))
        .unwrap_or(4);
