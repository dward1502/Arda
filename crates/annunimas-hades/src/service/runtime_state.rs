...
impl HadesService {
    ...
    pub(super) fn record_joulework(&self, operation: &str, result: &SweepResult) -> Result<()> {
        let baseline_joules = std::env::var("ANNUNIMAS_HADES_JOULE_BASELINE")
            .ok()
        ...
        let inference_provider =
            std::env::var("ANNUNIMAS_ACTIVE_PROVIDER").unwrap_or_else(|_| "none".to_owned());
        let inference_model =
            std::env::var("ANNUNIMAS_ACTIVE_MODEL").unwrap_or_else(|_| "none".to_owned());
        let inference_origin =
            std::env::var("ANNUNIMAS_ACTIVE_INFERENCE_ORIGIN").unwrap_or_else(|_| {
