...
fn sweep_discovers_orphan_and_status_updates() {
    ...
    // SAFETY: warden-owned by `annunimas-hades` test scaffolding — single-threaded
...
async fn sweep_sheds_excess_burst_work() {
    let _guard = env_guard();
    let dir = tempdir().expect("tempdir");
    std::env::set_var("ANNUNIMAS_HADES_SWEEP_MAX_CONCURRENCY", "1");
    let svc = HadesService::new(dir.path()).expect("service");
    ...
    std::env::remove_var("ANNUNIMAS_HADES_SWEEP_MAX_CONCURRENCY");
}
...
fn imports_human_scan_records_into_lifecycle_review_queue_without_destructive_action() {
    ...
    fs::write(
        ...
        concat!(
            ...
            "\"missing_frontmatter_keys\":[\"annunimas_contract\"],",
    ...
    let review: HumanLifecycleReviewItem =
        serde_json::from_str(queue_content.lines().next().expect("review line"))
            .expect("parse review");
    assert_eq!(review.contract, "annunimas.hades.human_lifecycle_review.v1");
...
fn lifecycle_audit_detects_stale_plan_archive_candidate_and_task_hygiene_without_delete() {
    ...
    assert_eq!(report.contract, "annunimas.hades.lifecycle_audit_report.v1");
...
fn organization_audit_and_plan_are_read_only_and_persist_artifacts() {
    ...
    assert_eq!(
        audit["contract"],
        "annunimas.hades.organization_audit_report.v1"
    );
    ...
    assert_eq!(plan["contract"], "annunimas.hades.organization_plan.v1");
...
fn organization_apply_requires_approved_packet_and_mutates_only_declared_candidates() {
    ...
    assert_eq!(
        receipt["contract"],
        "annunimas.hades.organization_apply_receipt.v1"
    );
