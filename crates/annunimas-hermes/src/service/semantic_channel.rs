...
fn upsert_arda_source_map_projection(
    ...
    let source_map_object = source_map.as_object_mut().ok_or_else(|| {
        AnnunimasError::Task("ARDA source map root must be a JSON object".to_string())
    })?;
    source_map_object
    ...
    let sections = sections_value.as_array_mut().ok_or_else(|| {
        AnnunimasError::Task("ARDA source map sections must be an array".to_string())
    })?;
