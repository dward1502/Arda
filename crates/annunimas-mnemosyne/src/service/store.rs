...
impl MnemosyneService {
    pub fn from_default_or_fallback() -> Result<Self> {
        ...
        let svc = match Self::new(&primary) {
            ...
            Err(err) => {
                ...
                Self::new(annunimas_root().join("data").join("mnemosyne"))?
            }
        };
    ...
    pub fn encode(&self, event: InformantEvent) -> Result<Option<RecallRecentEntry>> {
        ...
        self.emit_work_signal_background(
            ...
            annunimas_plutus::JouleWorkUnit::Reasoning,
...
fn default_root() -> PathBuf {
    if let Ok(custom) = std::env::var("ANNUNIMAS_MNEMOSYNE_HOME") {
        return PathBuf::from(custom);
    }
    annunimas_root().join("data/mnemosyne")
}

/// Phase 1 dual-write opt-in. Honors `ANNUNIMAS_CONTRACT_MEMORY_ROOT`
...
fn apply_contract_dual_write_from_env(svc: MnemosyneService) -> MnemosyneService {
    let raw = match std::env::var("ANNUNIMAS_CONTRACT_MEMORY_ROOT") {
        Ok(v) if !v.is_empty() => v,
        _ => return svc,
    };
    let path = if raw == "auto" {
        annunimas_root().join("core/state/memory")
    } else {
        PathBuf::from(raw)
    };
...
fn write_contract_memory_record(
    ...
    use annunimas_core::contract::{MemoryKind, MemoryRecord};
