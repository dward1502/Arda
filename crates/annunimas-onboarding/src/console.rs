...
use crate::helpers::now_utc;
use crate::io::{onboarding_run_dir, write_json, write_profile, write_readiness};
...
use crate::provider::provider_checklist;
use crate::readiness::build_readiness_projection;
use crate::service_plan::{build_approval_template, build_service_plan};
...
pub fn launch_console(
    ...
    let index_url = format!("http://{bind}:{port}/apps/onboarding-console/");
    ...
    write_json(
        &receipt,
        &json!({
            "contract": "annunimas.onboarding.launch_receipt.v1",
            "run": now_utc(),
            "address": addr,
            "url": index_url,
            "artifacts": {
                ...
                "private_config_proposal": "audit/onboarding-runs/latest-proposed-annunimas.env",
