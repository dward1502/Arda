...
use crate::constants::ONBOARDING_APPLY_RESULT_CONTRACT;
use crate::{
...
pub(crate) fn make_signup_hint(provider_id: &str, env_key: Option<&str>) -> ProviderSignupHint {
    ...
    match normalized.as_str() {
        "opencode" => {
            ...
            steps.push("Set OPENCODE_API_KEY in ~/.config/annunimas/annunimas.env".to_string());
        }
        "openrouter" => {
            steps.push("Open OpenRouter and generate an API key.".to_string());
            steps.push("Set OPENROUTER_API_KEY in ~/.config/annunimas/annunimas.env".to_string());
        }
        "openai" | "openai_sub" => {
            steps.push(
        ...
        "mistral" => {
            ...
            steps.push("Set MISTRAL_API_KEY in local annunimas.env.".to_string());
        }
        "groq" => {
            steps.push("Create Groq API key from Groq console.".to_string());
            steps.push("Set GROQ_API_KEY in local annunimas.env.".to_string());
        }
        "zai" => {
            steps.push("Create Z.AI key from portal.".to_string());
            steps.push("Set ZAI_API_KEY in local annunimas.env.".to_string());
        }
        "anthropic" => {
            steps.push("Create Anthropic credentials for Claude family models.".to_string());
            steps.push("Set ANTHROPIC_API_KEY in local annunimas.env.".to_string());
        }
        "cerebras" => {
            steps.push("Enable model route and key in Cerebras workspace.".to_string());
            steps.push("Set CEREBRAS_API_KEY in local annunimas.env.".to_string());
        }
        _ => {
            if let Some(key) = env_key {
                steps.push(format!("Set {key} in local annunimas.env."));
            }
