...
pub fn build_proposed_config(profile: &EnvironmentProfile, root: &Path) -> Result<PathBuf> {
    ...
    let receipt = json!({
        "contract": "annunimas.onboarding.proposed-config.v1",
        ...
        "machine_path_profile": {
            "annunimas_root": profile.paths.annunimas_root.value,
