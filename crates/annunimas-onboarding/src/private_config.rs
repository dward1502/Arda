...
use crate::helpers::{config_entry, now_utc};
use crate::io::{onboarding_run_dir, write_json};
use crate::provider::{provider_checklist, provider_env_keys};
use crate::types::*;

pub fn build_private_config_stage(profile: &EnvironmentProfile, root: &Path) -> PrivateConfigStage {
    let target_path = PathBuf::from(&profile.paths.config_dir.value).join("annunimas.env");
    let proposed_env_path = root
        .join("audit/onboarding-runs/latest-proposed-annunimas.env")
        .to_string_lossy()
        .to_string();
    ...
    let mut entries = vec![
        ...
        config_entry(
            "ANNUNIMAS_ROOT",
            profile.paths.annunimas_root.value.clone(),
            true,
            false,
            "Points tools at this checkout.",
        ),
        config_entry(
            "ANNUNIMAS_CONFIG_DIR",
