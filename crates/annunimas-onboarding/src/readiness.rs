...
use crate::helpers::{get_host_name, now_utc};
use crate::io::read_json_optional;
use crate::types::*;

pub fn l3_readiness_onboarding_checklist() -> Value {
    json!({
        "contract": "annunimas.onboarding.l3_readiness.v1",
        ...
        "verification": [
            "cargo run -p annunimas-cli --release -- export l3-readiness-projection",
            "jq '{status, flywheel, queue, hades_lifecycle}' core/state/l3_readiness_projection.json",
            "cargo run -p annunimas-cli --release -- pipeline flywheel-packet-readiness",
...
pub fn build_readiness_projection(
    ...
    checks.push(ReadinessCheck {
        check_id: "ANNUNIMAS_ROOT_PROTOCOL.md".to_string(),
        title: "Root protocol available".to_string(),
        status: if root.join("ANNUNIMAS_ROOT_PROTOCOL.md").exists() {
            "pass"
        } else {
            "warn"
        }
        .to_string(),
        severity: "high".to_string(),
        evidence: if root.join("ANNUNIMAS_ROOT_PROTOCOL.md").exists() {
            vec!["present: ANNUNIMAS_ROOT_PROTOCOL.md".to_string()]
        } else {
            vec!["missing: ANNUNIMAS_ROOT_PROTOCOL.md".to_string()]
        },
    ...
    checks.push(ReadinessCheck {
        ...
        evidence: vec![
            ...
            format!(
                "present: {}",
                if root.join("config/annunimas.template.toml").exists() {
                    "config/annunimas.template.toml"
                } else {
                    "missing config/annunimas.template.toml"
                }
            ),
        ],
        recommendation:
    ...
    ReadinessProjection {
        ...
        pass,
        warn,
        runner: Some("annunimas-onboarding".to_string()),
    }
}
