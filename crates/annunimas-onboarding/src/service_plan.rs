...
use crate::guided::build_guided_session;
use crate::helpers::{action_is_approved, action_receipt_path, make_apply_result, now_utc};
use crate::io::{
    build_proposed_config, onboarding_run_dir, write_json, write_profile, write_readiness,
};
use crate::prerequisites::build_prerequisite_report;
...
use crate::readiness::build_readiness_projection;
use crate::types::*;
...
pub fn build_service_plan(profile: &EnvironmentProfile, root: &Path) -> ServicePlan {
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding detect --write".to_string(),
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding readiness --output core/state/setup_console_readiness.json".to_string(),
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding provider-checklist".to_string(),
        target_path: Some(
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding device-scan".to_string(),
        target_path: Some(
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding prerequisites".to_string(),
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding private-config-stage".to_string(),
        target_path: Some(root.join("audit/onboarding-runs/latest-private-config-stage.json").to_string_lossy().to_string()),
        requires_human_gate: false,
        description: "Generate a secret-safe proposed annunimas.env artifact under audit/onboarding-runs without writing the private env file.".to_string(),
        risk: "read_only".to_string(),
    });
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding guided-session".to_string(),
        target_path: Some(
    ...
    actions.push(ServiceAction {
        ...
        command_hint: "cargo run -p annunimas-cli -- onboarding propose-config".to_string(),
        target_path: Some(root.join("audit/onboarding-runs").to_string_lossy().to_string()),
        requires_human_gate: false,
        description: "Render a machine-specific proposal from annunimas.template.toml into an audit artifact.".to_string(),
        risk: "read_only".to_string(),
    });
    ...
    if profile.safety.mutation_requires_human_gate {
        ...
        actions.push(ServiceAction {
            ...
            command_hint: "cargo run -p annunimas-cli -- onboarding apply-config --receipt <approved-receipt.json> --action-id onboarding.write_private_config_baseline".to_string(),
            target_path: Some(
            ...
            requires_human_gate: true,
            description: "Create or update the private annunimas.env baseline with non-secret staged values only; provider secrets remain placeholders for the operator.".to_string(),
        ...
        .any(|item| item.contains("CHARON_BASE_URL"))
    {
        actions.push(ServiceAction {
            ...
            command_hint: "export CHARON_BASE_URL=http://127.0.0.1:3001 and write to ~/.config/annunimas/annunimas.env".to_string(),
            target_path: Some(format!("{}/.config/annunimas/annunimas.env", env::var("HOME").unwrap_or_default())),
            requires_human_gate: true,
        ...
        .any(|item| item.contains("HERMES_BASE_URL"))
    {
        actions.push(ServiceAction {
            ...
            command_hint: "export HERMES_BASE_URL=http://127.0.0.1:8082 and write to ~/.config/annunimas/annunimas.env".to_string(),
            target_path: Some(format!("{}/.config/annunimas/annunimas.env", env::var("HOME").unwrap_or_default())),
            requires_human_gate: true,
        ...
        if !provider.missing_env.is_empty() {
            actions.push(ServiceAction {
                ...
                command_hint: provider
                    .missing_env
                    .iter()
                    .map(|env_key| format!("set {env_key} in ~/.config/annunimas/annunimas.env"))
                    .collect::<Vec<_>>()
                    .join(" && "),
                target_path: Some(format!("{}/.config/annunimas/annunimas.env", env::var("HOME").unwrap_or_default())),
                requires_human_gate: true,
