...
pub const PLUTUS_RUNTIME_SCHEMA_VERSION: &str = "annunimas.plutus.runtime.v1";
...
impl PlutusService {
    ...
    pub fn from_default_or_workspace_fallback() -> anyhow::Result<Self> {
        let home = std::env::var("ANNUNIMAS_PLUTUS_HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|_| annunimas_root().join("data/plutus"));
        Self::from_home(home)
    }
...
fn governance_record_for(
    ...
    task.status = annunimas_core::task::TaskStatus::Complete;
    PlutusGovernanceRecord {
