...
`annunimas-prometheus`'s autopilot service-health monitor so callers don't
...
- Not an agent supervisor. systemd already owns single-host service
  start/stop/restart, and `annunimas-prometheus` owns supervision *policy*.
  This crate is a typed client, nothing more.
- Not a fleet orchestrator — that lives in `annunimas-fleet`.

## History
This crate is the repurposed remains of `annunimas-supervisor`. The old
