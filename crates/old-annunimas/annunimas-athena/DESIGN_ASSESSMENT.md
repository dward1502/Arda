...
Grounded review of the Annunimas knowledge agent's architecture and
retrieval quality. Findings are sourced from the actual
`crates/old-annunimas/annunimas-athena/src` implementation.

> Status note: `annunimas-core` (the crate this code imports for
...
- **Digest — PARTIAL.** JSONL storage of deep extraction is fine for
  ...
  `annunimas-governance` / `annunimas-plutus` / `annunimas-mnemosyne`
