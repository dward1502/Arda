...
fn http_request_limit() -> usize {
    std::env::var("ANNUNIMAS_ATHENA_HTTP_MAX_CONCURRENCY")
        .ok()
...
fn http_request_body_limit() -> usize {
    std::env::var("ANNUNIMAS_ATHENA_HTTP_BODY_LIMIT_BYTES")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .filter(|value| *value > 0)
...
fn http_request_timeout_seconds() -> u64 {
    std::env::var("ANNUNIMAS_ATHENA_HTTP_REQUEST_TIMEOUT_SECS")
        .ok()
...
mod tests {
    ...
    async fn http_admission_gate_sheds_excess_burst_requests() {
        let _guard = env_guard();
        let dir = tempdir().expect("tempdir");
        std::env::set_var("ANNUNIMAS_ATHENA_HTTP_MAX_CONCURRENCY", "1");
        let store = AthenaStore::new(dir.path()).expect("store");
        let app = build_router(store);
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_HTTP_MAX_CONCURRENCY");
    }
    ...
    async fn http_rejects_oversized_request_body() {
        let _guard = env_guard();
        let dir = tempdir().expect("tempdir");
        std::env::set_var("ANNUNIMAS_ATHENA_HTTP_BODY_LIMIT_BYTES", "256");
        let store = AthenaStore::new(dir.path()).expect("store");
        let app = build_router(store);
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_HTTP_BODY_LIMIT_BYTES");
    }
    ...
    async fn http_enforces_request_timeout() {
        ...
        let _guard = env_guard();
        std::env::set_var("ANNUNIMAS_ATHENA_HTTP_REQUEST_TIMEOUT_SECS", "1");
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_HTTP_REQUEST_TIMEOUT_SECS");
    }
