use super::bootstrap_defaults::{default_providers, env_key_present};
...
pub(super) fn default_provider_intelligence_path() -> PathBuf {
    if let Ok(path) = std::env::var("ANNUNIMAS_PROVIDER_INTELLIGENCE_PATH") {
        return PathBuf::from(path);
    }
    super::paths::annunimas_root().join("core/state/provider_intelligence.json")
}

pub(super) fn default_tool_fit_model_intelligence_path() -> PathBuf {
    if let Ok(path) = std::env::var("ANNUNIMAS_TOOL_FIT_MODEL_INTELLIGENCE_PATH") {
        return PathBuf::from(path);
    }
    super::paths::annunimas_root().join("core/state/tool_fit_model_intelligence.json")
}
...
pub(super) fn load_providers_from_config(
    ...
    let cfg: ProviderConfigFile = toml::from_str(&raw).map_err(|e| AnnunimasError::Agent {
...
fn apply_provider_intelligence_overlay(
    ...
    let parsed: ProviderIntelligenceFile =
        serde_json::from_str(&raw).map_err(|e| AnnunimasError::Agent {
            agent: "charon".to_string(),
            message: format!(
...
pub(super) fn fleet_bootstrap_is_fresh(bootstrap: &FleetBootstrapFile) -> bool {
    ...
    let max_age_seconds = std::env::var("ANNUNIMAS_FLEET_BOOTSTRAP_MAX_AGE_SECONDS")
        .ok()
...
fn load_fleet_bootstrap_state(path: &Path) -> Result<Option<FleetBootstrapFile>> {
    ...
    let raw = fs::read_to_string(path)?;
    let parsed = serde_json::from_str(&raw).map_err(|e| AnnunimasError::Agent {
        agent: "charon".to_string(),
        message: format!(
...
mod tests {
  ...
  "models": [
    ...
    fn fleet_bootstrap_freshness_uses_positive_env_override() {
        ...
        std::env::set_var("ANNUNIMAS_FLEET_BOOTSTRAP_MAX_AGE_SECONDS", "1");
