use super::{AnnunimasError, JsonValue, ProviderState, Result};
use base64::Engine;
...
pub(super) fn codex_responses_timeout(default_timeout: Duration) -> Duration {
    std::env::var("ANNUNIMAS_CODEX_RESPONSES_TIMEOUT_SECONDS")
        .ok()
...
fn resolve_codex_access_token() -> Result<String> {
    ...
    let raw = std::fs::read_to_string(&path).map_err(|err| AnnunimasError::Agent {
