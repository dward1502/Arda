use super::{AnnunimasError, ProviderState, Result};
...
pub(super) async fn ensure_hermes_proxy(provider: &ProviderState) -> Result<String> {
    ...
    {
        let mut children = proxy_children().lock().map_err(|_| AnnunimasError::Agent {
...
pub(crate) fn hermes_proxy_base_url(provider: &ProviderState) -> String {
    provider
        ...
        .map(str::to_string)
        .unwrap_or_else(|| {
            let port = std::env::var("ANNUNIMAS_HERMES_PROXY_PORT")
                .ok()
...
fn resolve_hermes_bin(provider: &ProviderState) -> String {
    ...
    if let Ok(env) = std::env::var("ANNUNIMAS_HERMES_BIN") {
...
fn hermes_proxy_startup_attempts() -> usize {
    std::env::var("ANNUNIMAS_HERMES_PROXY_STARTUP_ATTEMPTS")
        .ok()
...
fn hermes_proxy_readiness_timeout_ms() -> u64 {
    std::env::var("ANNUNIMAS_HERMES_PROXY_READINESS_TIMEOUT_MS")
        .ok()
...
mod tests {
    use super::*;
    use crate::types::{ModelCapabilities, ModelState};
