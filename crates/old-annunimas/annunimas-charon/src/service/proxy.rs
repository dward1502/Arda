use super::{
    ...
    AnnunimasError, CharonService, GateAction, JsonValue, ProviderState, Result, RouteDecision,
    StdDuration,
};
use crate::service::adaptive_routing::classify_semantic_outcome;
use crate::service::state_mutation::ToolFitOutcome;
...
fn request_requires_tool_payload(req: &CharonRequestEnvelope) -> bool {
    req.options
...
pub(crate) fn strip_optional_tool_payload(req: &CharonRequestEnvelope, body: &mut JsonValue) {
        ...
        for _attempt in 0..max_attempts {
            ...
            if provider.driver == "codex_responses" {
                ...
                let mut request = client
                    ...
                    .header("User-Agent", "codex_cli_rs/0.0.0 (Annunimas Charon)")
...
pub(crate) fn proxy_timeout_for_provider(provider_id: &str, execution_lane: &str) -> StdDuration {
    if provider_id == "edge_backbone_coder" {
        ...
        return StdDuration::from_secs(
            std::env::var("ANNUNIMAS_CHARON_EDGE_CODER_TIMEOUT_SECONDS")
                .ok()
