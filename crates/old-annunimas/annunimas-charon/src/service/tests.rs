...
fn isolate_provider_sources(dir: &std::path::Path) {
    std::env::set_var(
        "ANNUNIMAS_PROVIDER_INTELLIGENCE_PATH",
    ...
    std::env::set_var(
        "ANNUNIMAS_FLEET_BOOTSTRAP_STATE",
...
fn clear_provider_sources() {
    std::env::remove_var("ANNUNIMAS_PROVIDER_INTELLIGENCE_PATH");
    std::env::remove_var("ANNUNIMAS_CHARON_PROVIDER_CONFIG");
    std::env::remove_var("ANNUNIMAS_FLEET_BOOTSTRAP_STATE");
}
...
async fn half_open_route_preserves_failure_streak_until_confirmed_success() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ANNUNIMAS_CHARON_HALF_OPEN_PROBE_STRIDE", "1");
    let dir = tempdir().expect("tempdir");
    ...
    let provider = providers
        ...
        .find(|provider| provider.id == "local_fallback")
        .expect("local_fallback provider");
    std::env::remove_var("ANNUNIMAS_CHARON_HALF_OPEN_PROBE_STRIDE");
    drop(_guard);
...
async fn per_agent_quota_blocks_only_exhausted_agent() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ANNUNIMAS_CHARON_AGENT_REQUESTS_PER_DAY", "1");
    let dir = tempdir().expect("tempdir");
    isolate_test_provider_config(dir.path());
    let svc = CharonService::new(dir.path()).expect("service");
    ...
    std::env::remove_var("ANNUNIMAS_CHARON_AGENT_REQUESTS_PER_DAY");
    drop(_guard);
...
async fn bandit_records_provider_result_and_affects_score() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ANNUNIMAS_CHARON_BANDIT_MIN_OBSERVATIONS", "1");
    std::env::set_var("ANNUNIMAS_CHARON_BANDIT_SCORE_WEIGHT", "8");
    let dir = tempdir().expect("tempdir");
    let svc = CharonService::new(dir.path()).expect("service");
    ...
    std::env::remove_var("ANNUNIMAS_CHARON_BANDIT_MIN_OBSERVATIONS");
    std::env::remove_var("ANNUNIMAS_CHARON_BANDIT_SCORE_WEIGHT");
    drop(_guard);
