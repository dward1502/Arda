use super::super::*;
use annunimas_mnemosyne::service::KnowledgeSeedRecallEntry;
...
pub(crate) fn handle_council(command: CouncilCommands) -> anyhow::Result<()> {
    match command {
        CouncilCommands::RecommendImprovements {
            ...
            let out = json!({
                "contract": "annunimas.council.recommend_improvements.v1",
...
pub(crate) fn handle_venture(command: VentureCommands) -> anyhow::Result<()> {
    match command {
        VentureCommands::Evaluate {
            ...
            let out = json!({
                "contract": "annunimas.venture.evaluate.v1",
                "generated_at_utc": Utc::now().to_rfc3339(),
                "scope": scope,
                "query": query,
                "limit": limit,
...
fn venture_score(
    athena_matches: &annunimas_athena::ingest::QueryResponse,
