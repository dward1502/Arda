...
use annunimas_tool_harness::types::{RiskLevel, SideEffectClass, ToolMetadata};
...
impl ToolEntry {
    pub fn harness_metadata(&self, tool_name: &str) -> ToolMetadata {
        ToolMetadata {
            ...
            input_schema_ref: format!("annunimas.tool.{tool_name}.input.v1"),
            output_schema_ref: format!("annunimas.tool.{tool_name}.output.v1"),
        ...
        if let Some(tools_table) = table.get("tools").and_then(|v| v.as_table()) {
            for (name, value) in tools_table {
                let entry: ToolEntry = value.clone().try_into().map_err(|e: toml::de::Error| {
                    crate::error::AnnunimasError::Config(format!(
