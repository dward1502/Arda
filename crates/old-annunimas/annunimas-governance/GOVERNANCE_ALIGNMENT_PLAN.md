...
# Annunimas Governance Alignment Plan

Scope: `crates/annunimas-governance/`
...
Move Annunimas governance from a single usefulness-efficiency proxy toward a layered alignment stack:
...
- `annunimas-prometheus/src/autopilot/oracle_gate.rs` now derives a deterministic Triad Philosopher verdict from ORACLE triad quorum evidence.
- `annunimas-prometheus/src/autopilot/governance_policy.rs` treats a non-proceed Triad Philosopher verdict as a triad-quorum hold, surfaces the verdict in governance reasons, and records compact evidence strings for runtime reports.
...
`annunimas-prometheus/src/autopilot/oracle_gate.rs` and `governance_policy.rs` now propagate the philosopher verdict into the operational triad quorum path:
...
- Focused integration test: `source scripts/runtime_build_env.sh && cargo test -p annunimas-governance --test alignment_stack` passed with 6 tests.
- Focused Prometheus tests:
  - `source scripts/runtime_build_env.sh && cargo test -p annunimas-prometheus triad_class_blocks_delegation_when_philosopher_does_not_proceed`
  - `source scripts/runtime_build_env.sh && cargo test -p annunimas-prometheus quorum_evidence_surfaces_triad_philosopher_verdict`
