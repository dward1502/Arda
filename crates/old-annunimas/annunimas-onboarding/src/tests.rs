use super::*;
use crate::constants::ONBOARDING_APPROVAL_CONTRACT;
use crate::helpers::{action_is_approved, secret_safe_preview};
use crate::private_config::merge_private_env;
...
fn service_action_contract_round_trips() {
    let action = ServiceAction {
        ...
        command_hint: "annunimas onboarding detect --write".to_string(),
...
fn private_env_merge_skips_secrets_and_preserves_unknown_lines() {
    let entries = vec![
        PrivateConfigEntry {
            key: "ANNUNIMAS_PROFILE".to_string(),
    ...
    assert!(merged.contains("ANNUNIMAS_PROFILE=\"local\""));
    assert!(!merged.contains("OPENAI_API_KEY"));
    assert_eq!(changed, vec!["ANNUNIMAS_PROFILE".to_string()]);
}
