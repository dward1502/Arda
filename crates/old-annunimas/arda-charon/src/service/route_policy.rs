...
pub(super) fn provider_eligible(p: &ProviderState, priority: &str, strict: bool) -> bool {
    ...
    // A half-open provider whose probe fraction is currently allowed must be
    // permitted through despite its failure streak: that is the entire point of
    // the half-open state (probe recovery before fully re-admitting). The probe
    // allowance was already enforced above by `provider_half_open_probe_allowed`,
    // so reaching here means the probe is permitted.
...
fn request_allows_low_context_tool_fallback(req: &CharonRequestEnvelope) -> bool {
    // An operator-forced provider is an explicit choice: honor its declared
    // model even if its context window is below the execution-lane target.
    // Otherwise forcing a small local provider would always yield
    // `no_compatible_model` and defeat the force directive.
    if req
        .options
...
fn request_forces_provider(req: &CharonRequestEnvelope, provider_id: &str) -> bool {
    req.options
        .get("force_provider_id")
        .and_then(serde_json::Value::as_str)
        .map(str::trim)
        .is_some_and(|forced| !forced.is_empty() && forced == provider_id)
}
...
pub(super) fn model_supports_request(
    ...
    if model.context_window < context_window_target {
        // An operator-forced provider is an explicit choice: do not reject its
        // model purely because the lane's context target exceeds the model's
        // window. Honoring the force directive matters more than the heuristic
        // context target here.
        if !request_forces_provider(req, provider_id) {
            return false;
        }
    }
