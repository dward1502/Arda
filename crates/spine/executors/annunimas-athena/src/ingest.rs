...
fn athena_error(message: impl Into<String>) -> AnnunimasError {
...
impl AthenaStore {
    pub fn new(root: impl AsRef<Path>) -> Result<Self> {
        ...
        let hades_queue_path = std::env::var("ANNUNIMAS_HADES_ACTION_QUEUE_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                crate::ingest::layout::annunimas_root().join("data/hades/action_queue.jsonl")
            });
        let warden_queue_path = std::env::var("ANNUNIMAS_WARDEN_QUEUE_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                crate::ingest::layout::annunimas_root().join("data/warden/informant_queue.jsonl")
            });
...
fn athena_ingest_limit() -> usize {
    ...
    std::env::var("ANNUNIMAS_ATHENA_INGEST_MAX_CONCURRENCY")
        .ok()
...
fn athena_deep_queue_limit() -> usize {
    ...
    std::env::var("ANNUNIMAS_ATHENA_DEEP_QUEUE_MAX_CONCURRENCY")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(default)
...
fn athena_crawl_limit() -> usize {
    ...
    std::env::var("ANNUNIMAS_ATHENA_CRAWL_MAX_CONCURRENCY")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(default)
...
mod tests {
    ...
    fn ingest_skips_workspace_triage_registry_for_noncanonical_store() {
        ...
        let previous_registry = std::env::var_os("ANNUNIMAS_KNOWLEDGE_TRIAGE_REGISTRY_PATH");
        ...
        let workspace_registry = crate::ingest::layout::annunimas_root()
        ...
        unsafe {
            ...
            if let Some(value) = previous_registry {
                std::env::set_var("ANNUNIMAS_KNOWLEDGE_TRIAGE_REGISTRY_PATH", value);
            } else {
                std::env::remove_var("ANNUNIMAS_KNOWLEDGE_TRIAGE_REGISTRY_PATH");
            }
        }
    ...
    fn ingest_emits_knowledge_triage_entry() {
        ...
        assert_eq!(entry["schema_version"], "annunimas.knowledge_triage.v1");
    ...
    fn status_exposes_knowledge_vault_scaffold() {
        ...
        assert_eq!(
            ...
            "docs/ANNUNIMAS_AUTONOMY_DOCTRINE.md"
        );
        assert_eq!(
    ...
    fn deep_analysis_recovers_missing_scholarly_metadata_from_ingest_url() {
        let _guard = env_guard();
        std::env::set_var("ANNUNIMAS_ATHENA_FORCE_OFFLINE_SCHOLARLY_METADATA", "true");
        let dir = tempdir().expect("tempdir");
        let store = AthenaStore::new(dir.path()).expect("store");
        let record = store
            .ingest(
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_FORCE_OFFLINE_SCHOLARLY_METADATA");
    }
    ...
    fn generate_planning_tasks_from_evidence() {
        ...
        // test process with no concurrent env reader at this point.
        unsafe {
            std::env::set_var("ANNUNIMAS_PROJECT_TASK_QUEUE_PATH", &queue_path);
        }
        let store = AthenaStore::new(dir.path()).expect("store");
        let record = store
            .ingest(
    ...
    fn lifecycle_events_write_hades_and_warden_queues() {
        ...
        // test process with no concurrent env reader at this point.
        unsafe {
            std::env::set_var("ANNUNIMAS_HADES_ACTION_QUEUE_PATH", &hades_queue);
        ...
        // test process with no concurrent env reader at this point.
        unsafe {
            std::env::remove_var("ANNUNIMAS_HADES_ACTION_QUEUE_PATH");
            std::env::remove_var("ANNUNIMAS_WARDEN_QUEUE_PATH");
        }
    }
    ...
    fn scrapling_fetch_markdown_supports_raw_html_shim() {
        ...
        // test process with no concurrent env reader at this point.
        unsafe {
            std::env::remove_var("ANNUNIMAS_SCRAPLING_RUNTIME_MODE");
        }
        let out = scrapling_fetch_markdown(
            "raw:<html><body><main><h1>Annunimas Scrapling</h1><p>Shim body.</p></main></body></html>",
            "fit",
            Some("knowledge"),
        )
        ...
        assert_eq!(
            out.provider,
            std::env::var("ANNUNIMAS_SCRAPLING_EXPECTED_PROVIDER")
                .unwrap_or_else(|_| "scrapling_shim".to_string())
        );
        assert!(out.markdown.contains("Annunimas Scrapling"));
    ...
    fn scrapling_fetch_markdown_fails_when_native_required_without_package() {
        ...
        // test process with no concurrent env reader at this point.
        unsafe {
            std::env::set_var("ANNUNIMAS_SCRAPLING_RUNTIME_MODE", "native_required");
        }
        let err = scrapling_fetch_markdown(
            "raw:<html><body><main><h1>Annunimas Scrapling</h1></main></body></html>",
            "fit",
            None,
        )
    ...
    async fn deep_analyze_sheds_excess_burst_work() {
        let _guard = env_guard();
        let dir = tempdir().expect("tempdir");
        std::env::set_var("ANNUNIMAS_ATHENA_DEEP_QUEUE_MAX_CONCURRENCY", "1");
        let store = AthenaStore::new(dir.path()).expect("store");
        let record = store
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_DEEP_QUEUE_MAX_CONCURRENCY");
    }
}
