...
use annunimas_core::{try_run_bounded, try_run_bounded_async};
...
pub async fn crawl4ai_fetch_markdown(
    ...
    let Some(result) = try_run_bounded_async("athena_crawl", athena_crawl_limit(), || async move {
        ...
        let value: serde_json::Value =
            response.json().await.map_err(|e| AnnunimasError::Agent {
                agent: "athena".to_owned(),
...
fn resolve_scrapling_runtime_mode() -> String {
    match std::env::var("ANNUNIMAS_SCRAPLING_RUNTIME_MODE") {
...
fn fetch_scrapling_html(url: &str) -> Result<String> {
    ...
    let response = client
        .get(url)
        .header("User-Agent", "Annunimas-Scrapling-Rust/0.1")
        .send()
        .map_err(|e| AnnunimasError::Agent {
            agent: "athena".to_owned(),
    ...
    response.text().map_err(|e| AnnunimasError::Agent {
        agent: "athena".to_owned(),
