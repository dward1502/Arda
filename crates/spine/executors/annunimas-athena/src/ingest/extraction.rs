...
use annunimas_core::llm::{ChatMessage, ChatRequest, LlmProvider};
use serde::{Deserialize, Serialize};
use std::sync::Arc;

use super::{athena_error, ShallowAnalysis};

const SYSTEM_PROMPT: &str = "You are ATHENA, the knowledge synthesis arm of the Annunimas autonomous agent system. \
Annunimas is a Rust workspace of sovereign crates (charon=router, hermes=comm, athena=knowledge, hades=ops, prometheus=planner, plutus=economy, warden=governance, mnemosyne=memory). \
...
pub struct ExtractedKnowledge {
    ...
    #[serde(default)]
    pub applicability_to_annunimas: String,
    #[serde(default)]
...
{\n\
  ...
  \"applicability_to_annunimas\": string,       // 1-3 sentences: how this could integrate into Annunimas (concrete, not generic)\n\
  \"integration_hooks\": [string],              // specific Annunimas crates/files/abstractions to attach to (e.g. crates/annunimas-charon/src/router.rs)\n\
...
mod tests {
    ...
    fn parses_strict_json_response() {
        let body = r#"{"concepts":["a","b"],"patterns":[],"novel_ideas":["x"],"applicability_to_annunimas":"integrate as router plugin","integration_hooks":["crates/charon"],"comparable_systems":["foo"],"risks_or_concerns":["mem"],"confidence_self_report":0.8,"summary_one_paragraph":"summary"}"#;
        let k = parse_response(body);
        assert_eq!(k.concepts, vec!["a", "b"]);
        assert_eq!(k.applicability_to_annunimas, "integrate as router plugin");
    ...
    fn parses_json_inside_markdown_fence() {
        let body = "```json\n{\"concepts\":[\"a\"],\"patterns\":[],\"novel_ideas\":[],\"applicability_to_annunimas\":\"x\",\"integration_hooks\":[],\"comparable_systems\":[],\"risks_or_concerns\":[],\"confidence_self_report\":0.5,\"summary_one_paragraph\":\"s\"}\n```";
        let k = parse_response(body);
    ...
    fn parses_json_with_preamble() {
        let body = "Here is the extraction:\n{\"concepts\":[\"x\"],\"patterns\":[],\"novel_ideas\":[],\"applicability_to_annunimas\":\"\",\"integration_hooks\":[],\"comparable_systems\":[],\"risks_or_concerns\":[],\"confidence_self_report\":0.3,\"summary_one_paragraph\":\"\"}\nHope this helps.";
        let k = parse_response(body);
    ...
    fn clamps_confidence_above_one() {
        let body = r#"{"concepts":[],"patterns":[],"novel_ideas":[],"applicability_to_annunimas":"","integration_hooks":[],"comparable_systems":[],"risks_or_concerns":[],"confidence_self_report":7.0,"summary_one_paragraph":""}"#;
        let k = parse_response(body);
