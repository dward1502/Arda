...
fn ipc_connection_limit() -> usize {
    std::env::var("ANNUNIMAS_ATHENA_IPC_MAX_CONCURRENCY")
        .ok()
...
fn ipc_connect_timeout() -> Duration {
    let secs = std::env::var("ANNUNIMAS_ATHENA_IPC_CONNECT_TIMEOUT_SECS")
        .ok()
...
fn ipc_idle_timeout() -> Duration {
    let secs = std::env::var("ANNUNIMAS_ATHENA_IPC_IDLE_TIMEOUT_SECS")
        .ok()
        .and_then(|raw| raw.parse::<u64>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(30);
...
fn ipc_io_timeout() -> Duration {
    ...
    // Override via ANNUNIMAS_ATHENA_IPC_IO_TIMEOUT_SECS for fast-path tools
    // that need tighter timeouts.
    let secs = std::env::var("ANNUNIMAS_ATHENA_IPC_IO_TIMEOUT_SECS")
        .ok()
        .and_then(|raw| raw.parse::<u64>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(120);
...
async fn handle_connection(stream: UnixStream, store: AthenaStore) -> Result<()> {
    ...
    loop {
        ...
        timeout(io_deadline, writer.write_all(&encoded))
            .await
            .map_err(|_| AnnunimasError::Agent {
                agent: "athena".to_string(),
...
mod tests {
    ...
    async fn ipc_client_connect_times_out_on_dead_socket() {
        ...
        std::env::set_var("ANNUNIMAS_ATHENA_IPC_CONNECT_TIMEOUT_SECS", "1");
        ...
        std::env::remove_var("ANNUNIMAS_ATHENA_IPC_CONNECT_TIMEOUT_SECS");
    }

    #[tokio::test]
    async fn ipc_server_closes_idle_connection() {
        let _guard = env_guard();
        std::env::set_var("ANNUNIMAS_ATHENA_IPC_IDLE_TIMEOUT_SECS", "1");
        ...
        let listener = match UnixListener::bind(&socket_path) {
            ...
            Err(err) => {
                let msg = err.to_string();
                if msg.contains("Operation not permitted") || msg.contains("Permission denied") {
                    std::env::remove_var("ANNUNIMAS_ATHENA_IPC_IDLE_TIMEOUT_SECS");
                    return;
                }
                panic!("bind: {err}");
            }
