use annunimas_athena::human::{scan_human_root, HumanIngestionRecord};
use annunimas_athena::ingest::AthenaStore;
use annunimas_core::error::Result;
use annunimas_core::llm::{ChatRequest, ChatResponse, LlmProvider};
...
fn uncertainty_sampler_uses_llm_score_for_query_matches() {
    ...
    let root = temp.path().join("annunimas-root");
    ...
    let _root_env = EnvVarGuard::set_path("ANNUNIMAS_ROOT", &root);
    let _human_env = EnvVarGuard::set_path("ANNUNIMAS_ATHENA_HUMAN_LIBRARY_ROOT", &human_library);
    let _machine_env =
        EnvVarGuard::set_path("ANNUNIMAS_ATHENA_MACHINE_LIBRARY_ROOT", &machine_library);
    let _hades_env = EnvVarGuard::set_path("ANNUNIMAS_HADES_ACTION_QUEUE_PATH", &hades_queue);
    let _warden_env = EnvVarGuard::set_path("ANNUNIMAS_WARDEN_QUEUE_PATH", &warden_queue);
    let _bacon_machine_env = EnvVarGuard::set_path("ANNUNIMAS_BACON_LITE_LOG_PATH", &bacon_machine);
    let _bacon_human_env = EnvVarGuard::set_path("ANNUNIMAS_BACON_LITE_HUMAN_PATH", &bacon_human);
...
fn human_root_scan_classifies_fixture_and_preserves_provenance() {
    ...
    fs::write(
        &note_path,
        r#"---
annunimas_contract: human.note.v1
...
affected_paths: crates/annunimas-athena, crates/annunimas-prometheus
privacy: internal
