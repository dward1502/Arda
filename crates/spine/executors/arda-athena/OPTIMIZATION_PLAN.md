...
Scope: `crates/arda-athena/` — knowledge-ingest, query, deep-analysis, and synthesis agent. `lib.rs` (agent entrypoint, model routing, LLM execution), `ingest.rs` + `ingest/` (13 submodules: crawl, importers, interceptor, io, scholarly, source, policy, routing, query, views, deep, remediation, observability, layout), `transport/` (IPC + optional HTTP/SSE).
...
- Touch: `crates/arda-athena/Cargo.toml`, any reqwest call sites that use 0.11-only APIs.
...
- Default LLM endpoint is **Charon's `/v1` model router** (`http://127.0.0.1:5110/v1`, model `auto`). Overrides: `ARDA_ATHENA_LLM_BASE_URL`, `ARDA_ATHENA_LLM_MODEL`, `ARDA_ATHENA_LLM_API_KEY_ENV`, or `ARDA_ATHENA_LLM_USE_CONFIG=1` to fall back to `config/default.toml`.
- Bumped default IPC io-timeout from 15s → 120s in `transport/ipc.rs` so the daemon path can complete LLM extraction (override via `ARDA_ATHENA_IPC_IO_TIMEOUT_SECS`).
...
For `SourceType::GithubRepo` / `GithubFile` URLs, ingest now fetches via the GitHub REST API and populates structured shallow fields (title, summary, language, license, key_dependencies, stars, README excerpt, topics) instead of writing a URL-only placeholder. Auth via `ARDA_GITHUB_TOKEN` (raises 60→5000 req/hr). Workspace `Cargo.toml` with no root deps now descends into members (inline and glob) to surface real dependencies. Offline-deterministic via `ARDA_ATHENA_FORCE_OFFLINE_GITHUB_METADATA=true`.

- New module: `crates/arda-athena/src/ingest/github.rs` (parser, REST client, manifest parsers for cargo/npm/pyproject/requirements/gomod).
...
|------|--------|-------------------|
| A1   | done   | `cargo tree -p arda-athena` shows only `reqwest v0.12.28`; workspace duplicate 0.11 users in core/governance were bumped. |
...
```bash
cargo fmt -p arda-athena
cargo check -p arda-athena
cargo test -p arda-athena malformed_policy_readiness_gauge_renders_and_snapshots -- --nocapture
cargo test -p arda-athena http_contract_metrics_endpoint_exports_prometheus_text -- --nocapture
cargo test -p arda-athena ipc_round_trip_ingest_query_status -- --nocapture
source scripts/runtime_build_env.sh && cargo tree -p arda-athena | grep 'reqwest v' | sort -u
```

Observed results:

- Formatting completed successfully.
- `cargo check -p arda-athena` completed successfully.
...
3. Verification target: add unit coverage for append helper behavior and shared-client configuration, then run `cargo fmt -p arda-athena`, `cargo check -p arda-athena`, and focused Athena tests.
