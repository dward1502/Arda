...
fn uncertainty_sampler_uses_llm_score_for_query_matches() {
    ...
    let _human_env = EnvVarGuard::set_path("ARDA_ATHENA_HUMAN_LIBRARY_ROOT", &human_library);
    let _machine_env =
        EnvVarGuard::set_path("ARDA_ATHENA_MACHINE_LIBRARY_ROOT", &machine_library);
...
affected_paths: crates/arda-athena, crates/arda-prometheus
privacy: internal
