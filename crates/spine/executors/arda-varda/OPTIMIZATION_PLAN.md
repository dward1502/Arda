...
```bash
cargo fmt -p arda-varda
cargo check -p arda-varda
cargo test -p arda-varda malformed_policy_readiness_gauge_renders_and_snapshots -- --nocapture
cargo test -p arda-varda http_contract_metrics_endpoint_exports_prometheus_text -- --nocapture
cargo test -p arda-varda ipc_round_trip_ingest_query_status -- --nocapture
source scripts/runtime_build_env.sh && cargo tree -p arda-varda | grep 'reqwest v' | sort -u
```

Observed results:

- Formatting completed successfully.
- `cargo check -p arda-varda` completed successfully.
