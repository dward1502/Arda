...
fn pressure_admission_enabled() -> bool {
    std::env::var("ARDA_PRESSURE_ADMISSION_ENABLED")
...
fn current_pressure_snapshot() -> Arc<PressureSnapshot> {
    let ttl_ms = std::env::var("ARDA_PRESSURE_ADMISSION_CACHE_MS")
        .ok()
...
fn load_pressure_snapshot() -> PressureSnapshot {
    let budget_path = std::env::var("ARDA_PRESSURE_ADMISSION_BUDGET_PATH")
        .or_else(|_| std::env::var("ARDA_LAUNCH_PREFLIGHT_BUDGET_PATH"))
        .map(PathBuf::from)
        .unwrap_or_else(|_| workspace_root().join("core/state/runtime_budget_policy.json"));
    let pressure_path = std::env::var("ARDA_PRESSURE_ADMISSION_PRESSURE_PATH")
        .or_else(|_| std::env::var("ARDA_LAUNCH_PREFLIGHT_PRESSURE_PATH"))
        .map(PathBuf::from)
...
fn emit_shed_receipt(
    ...
    let path = std::env::var("ARDA_PRESSURE_ADMISSION_RECEIPTS_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
