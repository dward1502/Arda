// sigil: ∇ ◈ ↝
//
// `annunimas-cli loop …` — Phase 1 autonomy loop driver.
//
...
use annunimas_core::contract::{Goal, GoalPriority};
...
fn build_dispatch_estimator() -> annunimas_plutus::EstimatorMeter {
    let path = annunimas_root().join("config").join("joule_tariffs.toml");
    if path.exists() {
        match annunimas_plutus::EstimatorMeter::load_from_path(&path) {
            Ok(m) => {
    ...
    annunimas_plutus::EstimatorMeter::with_default_tariffs()
}

/// Load `<root>/config/governance_gates.yaml`. Falls back to the
/// permissive (no-op) gate if missing or malformed.
fn build_governance_gates() -> annunimas_core::governance_gates::GovernanceGates {
    let path = annunimas_root()
        .join("config")
        .join("governance_gates.yaml");
    if path.exists() {
        match annunimas_core::governance_gates::GovernanceGates::load_from_path(&path) {
            Ok(g) => {
    ...
    annunimas_core::governance_gates::GovernanceGates::permissive()
}
...
fn warden_enforce(args: WardenEnforceArgs) -> anyhow::Result<()> {
    ...
    let dry_run = !apply;
    let halt_path = state_root_path.join(annunimas_core::loop_engine::HALT_FILE_NAME);
    ...
    let report = json!({
        "contract": "annunimas.warden.security_monitoring_enforcement.v1",
...
fn warden_status(state_root_arg: Option<PathBuf>, limit: usize) -> anyhow::Result<()> {
    ...
    let alerts = annunimas_core::loop_alerts::read_recent(&state, limit)?;
...
fn halt_path(state_root_arg: Option<PathBuf>) -> PathBuf {
    let root = annunimas_root();
    let state_root_path = state_root_arg.unwrap_or_else(|| root.join("core/state"));
    state_root_path.join(annunimas_core::loop_engine::HALT_FILE_NAME)
}
...
fn status(state_root_arg: Option<PathBuf>) -> anyhow::Result<()> {
    use annunimas_core::contract::{GoalStatus, MemoryKind};
    use annunimas_core::state;
    ...
    let halt = state
        .root()
        .join(annunimas_core::loop_engine::HALT_FILE_NAME);
    if halt.exists() {
    ...
    println!(
        "            snapshot: {}",
        annunimas_core::loop_economy::snapshot_path(&state).display()
    );
...
fn loop_status_economy_snapshot(
    state: &StateRoot,
) -> std::io::Result<annunimas_core::loop_economy::LoopEconomySnapshot> {
    annunimas_core::loop_economy::build_snapshot(state)
}
...
fn tick(state_root_arg: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let queue_path = state.queue_path(&root);
    let plan_pass = annunimas_prometheus::planner::run(&state, Some(&queue_path))
    ...
    let triad = annunimas_governance::LiveTriad::new();
    let bid_board = annunimas_core::loop_engine::StaticBidBoard;
    let gates = build_governance_gates();
    let dispatch_pass = annunimas_core::loop_engine::dispatch_full(
        &state,
        &queue_path,
        annunimas_core::loop_engine::DEFAULT_DISPATCH_CAP_PER_TICK,
        &estimator,
        &triad,
        &bid_board,
        &gates,
    ...
    let reflect_pass = annunimas_core::loop_engine::reflect(&state, &queue_path)
    ...
    let reflections =
        state::list_reflections(&state).map_err(|e| anyhow::anyhow!("list reflections: {}", e))?;
    let mut alerts = annunimas_core::loop_alerts::analyze_tick(&dispatch_pass, &reflections);
    alerts.extend(
        annunimas_core::loop_alerts::analyze_chaos_log(&state)
            .map_err(|e| anyhow::anyhow!("chaos alerts: {}", e))?,
    );
    let alerts_written = annunimas_core::loop_alerts::append_alerts(&state, &alerts)
        .map_err(|e| anyhow::anyhow!("warden alerts: {}", e))?;
    println!("warden:   {} alert(s) written", alerts_written);

    let economy = annunimas_core::loop_economy::write_snapshot(&state)
        .map_err(|e| anyhow::anyhow!("loop economy snapshot: {}", e))?;
    println!(
...
mod tests {
    ...
    fn loop_status_economy_snapshot_does_not_write_projection_file() {
        let root = unique_temp_dir("annunimas-loop-status-economy");
        let state = StateRoot::new(&root);
        let snapshot_path = annunimas_core::loop_economy::snapshot_path(&state);
    ...
    fn warden_enforce_dry_run_reports_critical_without_writing_halt() {
        let root = unique_temp_dir("annunimas-warden-enforce");
        ...
        assert_eq!(
            ...
            Some("annunimas.warden.security_monitoring_enforcement.v1")
        );
