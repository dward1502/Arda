...
fn is_public_repo_skeleton_task(task: &Value) -> bool {
    ...
    field("origin") == "session_pivot"
        ...
        && (field("risk") == "safe-local"
            ...
            || field("risk").is_empty())
        && (field("harness") == "annunimas-cli" || field("harness").is_empty())
...
fn flywheel_prompt(task: &Value, plan_text: &str) -> String {
    ...
    format!(
        "You are executing an Annunimas Flywheel work packet.\n\n\
Task ID: {}\n\
Title: {}\n\
Owner: {}\n\
Packet: {}\n\
...
fn flywheel_dispatch_receipt(
    ...
    let meta = task.get("meta").cloned().unwrap_or_else(|| json!({}));
    json!({
        "schema_version": "annunimas.flywheel.dispatch_receipt.v1",
        "ts_utc": now_utc(),
        ...
        "preflight": preflight,
        "dispatch": dispatch,
        "receipt_writer": "annunimas-cli pipeline flywheel-dispatch",
    })
}
...
fn flywheel_review_receipt(
    ...
    let meta = task.get("meta").cloned().unwrap_or_else(|| json!({}));
    json!({
        "schema_version": "annunimas.flywheel.review_receipt.v1",
        "ts_utc": now_utc(),
        "task_id": task.get("id").cloned().unwrap_or(Value::Null),
        "plan": meta.get("plan").cloned().unwrap_or(Value::Null),
        "packet_id": meta.get("packet_id").cloned().unwrap_or(Value::Null),
        ...
        "notes": notes,
        "dry_run": dry_run,
        "receipt_writer": "annunimas-cli pipeline flywheel-review-receipt",
    })
}
...
fn cli_command(args: &[&str]) -> anyhow::Result<CommandEvent> {
    Ok(CommandEvent {
        display: std::iter::once("annunimas-cli".to_string())
...
fn emit_async_user_intake_tasks() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.async-user-intake-queue.v1",
...
fn run_async_user_intake_executor() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.async-user-intake-runtime.v1",
        "generated_at_utc": now_utc(),
...
fn emit_human_corpus_digest_tasks() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.human-corpus-digest-tasks.v1",
        "generated_at_utc": now_utc(),
...
fn reconcile_human_corpus_digest_tasks() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.human-corpus-digest-reconciliation.v1",
        "generated_at_utc": now_utc(),
...
fn emit_source_absorption_tasks() -> anyhow::Result<Value> {
    ...
    let promote_now_total = candidates
        ...
        .count();
    let payload = json!({
        "schema_version": "annunimas.source-absorption-execution.v1",
        "generated_at_utc": now_utc(),
...
fn run_source_absorption_executor() -> anyhow::Result<Value> {
    ...
    let remaining = latest
        ...
        .count();
    let payload = json!({
        "schema_version": "annunimas.source-absorption-autopilot.v1",
        "generated_at_utc": now_utc(),
...
fn reconcile_source_absorption_downstream() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.source-absorption-downstream-reconciliation.v1",
        "generated_at_utc": now_utc(),
...
fn platform_os_boundary_review_payload(root: &Path) -> anyhow::Result<Value> {
    ...
    Ok(json!({
        "schema_version": "annunimas.platform_os_boundary_review.v1",
        "generated_at_utc": now_utc(),
...
fn project_task_executor() -> anyhow::Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.project-task-executor.v1",
        "generated_at_utc": now_utc(),
...
fn flywheel_dispatch(task_id: Option<&str>, write: bool) -> anyhow::Result<Value> {
    ...
    Ok(json!({
        "schema_version": "annunimas.flywheel.dispatch_report.v1",
        "task_id": task.get("id").cloned().unwrap_or(Value::Null),
        "packet": classified,
        "dry_run": dry_run,
        "write": write,
...
fn flywheel_review_receipt_command(
    ...
    Ok(json!({
        "schema_version": "annunimas.flywheel.review_report.v1",
        "task_id": task_id,
        "dry_run": dry_run,
        "write": write,
        "receipt_written": receipt_written,
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "annunimas-cli-flywheel-{name}-{}-{unique}",
            std::process::id()
        ));
    ...
    fn platform_os_boundary_review_payload_is_read_only_gate() {
        ...
        fs::write(
            ...
            r#"[workspace]
members = [
    "crates/annunimas-core",
    "crates/annunimas-cli",
    "crates/annunimas-human",
]
"#,
        )
        .expect("manifest");
        ...
        fs::write(
            ...
            serde_json::to_string(&json!({
                "frozen_core_members": ["annunimas-core", "annunimas-cli"]
            }))
            .expect("audit json"),
        )
        .expect("audit");
        ...
        assert_eq!(
            ...
            "annunimas.platform_os_boundary_review.v1"
        );
        assert_eq!(
        ...
        assert!(payload["non_core_workspace_members"]
            ...
            .expect("non-core")
            .iter()
            .any(|member| member.as_str() == Some("annunimas-human")));
    ...
    fn flywheel_readiness_executor_rule_is_narrow() {
        let selector = task(
            ...
            json!({
                ...
                "verify": "cargo_test_annunimas_cli_flywheel_and_project_task_executor",
                "receipt_surface": "core/state/flywheel_packet_runtime.json",
            }),
        );
    ...
    fn flywheel_prompt_contains_packet_boundaries() {
        ...
        meta["expected_files"] = json!("crates/annunimas-cli/src/commands/pipeline.rs");
        let packet = task("packet", "queued", meta);
    ...
    fn flywheel_dispatch_receipt_preserves_dry_run_and_digest() {
        ...
        assert_eq!(
            ...
            "annunimas.flywheel.dispatch_receipt.v1"
        );
    ...
    fn flywheel_review_receipt_records_evidence_fields() {
        ...
        let changed_files = vec!["crates/annunimas-cli/src/commands/pipeline.rs".to_string()];
        let verification = vec!["cargo test -p annunimas-cli flywheel --all-targets".to_string()];
        ...
        assert_eq!(
            receipt["schema_version"],
            "annunimas.flywheel.review_receipt.v1"
        );
        assert_eq!(receipt["task_id"], "packet");
        assert_eq!(
            receipt["changed_files"][0],
            "crates/annunimas-cli/src/commands/pipeline.rs"
        );
