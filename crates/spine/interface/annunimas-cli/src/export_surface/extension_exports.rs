...
pub(crate) fn export_openfang_alignment_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.openfang.alignment.v1",
        ...
        "pattern_extraction": {
            "hands_to_spawnable_crates": {
                ...
                "annunimas_adaptation": "crate-spawn templates with sovereign task, metrics, ARDA, and control-plane hooks from first boot",
                "priority": "high",
            },
            "wire_to_network_onboarding": {
                "openfang_pattern": "network-native OFP onboarding and mutual auth",
                "annunimas_adaptation": "keep Unix sockets local, Tailscale internal, and fold network-native node onboarding above the sovereign local layer",
                "priority": "high",
            },
            "skills_marketplace_boundary": {
                "openfang_pattern": "skills and marketplace framing around reusable packages",
                "annunimas_adaptation": "align with `.aipkg` as open package law and keep marketplace economics out of core truth",
                "priority": "high",
            },
            "desktop_embodiment": {
                "openfang_pattern": "Tauri desktop as a first-class native surface",
                "annunimas_adaptation": "carry forward into ARDA embodied interface and Pepper's Ghost controller surfaces",
                "priority": "medium",
            },
            "security_stack": {
                "openfang_pattern": "explicit signing, taint tracking, sandboxing, and audit trail",
                "annunimas_adaptation": "map to WARDEN, HADES, `.aipkg` receipts/signatures, and control-plane lockdown projections",
                "priority": "high",
            },
        },
        "annunimas_follow_ons": [
...
pub(crate) fn export_agentforge_alignment_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.agentforge.alignment.v1",
        "generated_at_utc": now_utc(),
            ...
            "crate_boundaries": [
                {
                    ...
                    "surfaces": [
                        ...
                        "annunimas-cli utility create-crate-spawn-blueprint",
                    ],
...
pub(crate) fn export_eliza_alignment_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.eliza.alignment.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_agent_framework_alignment_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.agent-framework-alignment.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_extension_surface_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.extension-surface-contract.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_extension_activation_backlog_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.extension-activation-backlog.v1",
        "generated_at_utc": now_utc(),
