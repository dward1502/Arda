...
pub(crate) fn export_fleet_bootstrap_state_impl() -> Result<Value> {
    ...
    let unexpected_offline_total = node_rows
        ...
        .count();
    let bootstrap = json!({
        "schema_version": "annunimas.fleet-bootstrap.v1",
    ...
    let edge_payload = json!({
        "schema_version": "annunimas.edge-endpoint-verification.v3",
        "generated_at_utc": generated_at,
        "authority": "fleet_bootstrap_state",
        "probe_status": "ok",
        "summary": {
    ...
    let recovery_payload = json!({
        "schema_version": "annunimas.fleet-bootstrap-recovery.v1",
        "generated_at_utc": generated_at,
        "authority": "fleet_bootstrap_state",
        "recovery_enabled": env_flag("ANNUNIMAS_FLEET_BOOTSTRAP_RECOVER", false),
        "recover_wait_seconds": env_int("ANNUNIMAS_FLEET_BOOTSTRAP_RECOVER_WAIT_SECONDS", 8),
        "recover_retries": env_int("ANNUNIMAS_FLEET_BOOTSTRAP_RECOVER_RETRIES", 1),
        "summary": {
            "restart_attempted_total": restart_attempted_total,
            "recovered_total": recovered_total,
            "restart_failed_total": restart_failed_total,
...
fn try_restart(node: &toml::value::Table, status: Option<&Value>, root: &Path) -> Option<Value> {
    if !env_flag("ANNUNIMAS_FLEET_BOOTSTRAP_RECOVER", false) {
        return None;
    }
...
fn node_status(node: &toml::value::Table, tailnet: Option<&Value>, root: &Path) -> Value {
    ...
    if !initial_probe
        ...
        && !intentional_offline
    {
        let recover_retries = env_int("ANNUNIMAS_FLEET_BOOTSTRAP_RECOVER_RETRIES", 1).max(1);
...
pub(crate) fn export_fleet_steward_actions_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.fleet-steward-actions.v1",
...
pub(crate) fn export_fleet_steward_write_intents_impl() -> Result<Value> {
    ...
    let intents: Vec<Value> = actions
        ...
        .collect();
    let payload = json!({
        "schema_version": "annunimas.fleet-steward-write-intents.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_fleet_power_guard_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.fleet-power-guard.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_fleet_identity_reconciliation_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.fleet-identity-reconciliation.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_fleet_capability_ranking_impl() -> Result<Value> {
    ...
    for operator in operator_nodes {
        ...
        if local_gpu_model.is_some() {
            constraints.push(json!(
                "local_nvml_runtime_not_currently_observable_from_annunimas_context"
            ));
        }
    ...
    let operator = ranking_rows
        ...
        .unwrap_or_else(|| json!({}));
    let payload = json!({
        "schema_version": "annunimas.fleet-capability-ranking.v1",
        "generated_at_utc": now_utc(),
