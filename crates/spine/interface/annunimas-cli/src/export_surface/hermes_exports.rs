...
pub(crate) fn export_matrix_boardroom_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.matrix-boardrooms.v1",
...
pub(crate) fn export_federated_comms_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.federated-comms.v1",
        "generated_at_utc": now_utc(),
    ...
    let runtime = json!({
        "schema_version": "annunimas.federated-comms-runtime.v1",
        "generated_at_utc": now_utc(),
        ...
        "matrix": {
            ...
            "access_token_env": matrix.get("access_token_env").and_then(toml::Value::as_str).unwrap_or("ANNUNIMAS_MATRIX_ACCESS_TOKEN"),
        ...
        "selinux": {
            ...
            "runtime_socket_root": selinux.get("runtime_socket_root").and_then(toml::Value::as_str).unwrap_or("/run/user/$UID/annunimas"),
        },
        "bitmesh": {
...
fn framework_entry(root: &std::path::Path, name: &str, path: &str) -> Option<Value> {
    ...
    Some(json!({
        ...
        "annunimas_adaptation": payload.get("annunimas_adaptation").cloned().unwrap_or(Value::Null),
        "pattern_extraction": payload.get("pattern_extraction").cloned().unwrap_or(Value::Null),
        "follow_ons": payload.get("annunimas_follow_ons").cloned().unwrap_or(Value::Null),
    }))
}
...
fn infer_disposition(
    ...
    let (mode, rationale) = if activation_status == "active_in_system" {
        (
            "active_in_system",
            "This repo is already live inside Annunimas runtime contracts.",
        )
...
pub(crate) fn export_github_repo_integration_impl() -> Result<Value> {
    ...
    let registry_tools = registry
        ...
        .unwrap_or(tools.len());
    let payload = json!({
        "schema_version": "annunimas.github-repo-integration.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_embodied_controller_runtime_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.embodied-controller-runtime.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_edge_enrollment_plan_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "annunimas.edge-enrollment-plan.v1",
        "generated_at_utc": now_utc(),
