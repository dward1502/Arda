...
pub(crate) fn runtime_surface() -> serde_json::Value {
    ...
    json!({
        "paths": {
            "ANNUNIMAS_ATHENA_HOME": env_or("ANNUNIMAS_ATHENA_HOME", "data/athena"),
            "ANNUNIMAS_PROMETHEUS_HOME": env_or("ANNUNIMAS_PROMETHEUS_HOME", "data/prometheus"),
            "ANNUNIMAS_CHARON_HOME": env_or("ANNUNIMAS_CHARON_HOME", "data/charon"),
            "ANNUNIMAS_HADES_HOME": env_or("ANNUNIMAS_HADES_HOME", "data/hades"),
            "ANNUNIMAS_HERMES_HOME": env_or("ANNUNIMAS_HERMES_HOME", "data/hermes"),
            "ANNUNIMAS_MNEMOSYNE_HOME": env_or("ANNUNIMAS_MNEMOSYNE_HOME", "data/mnemosyne"),
            "ANNUNIMAS_APOLLO_HOME": env_or("ANNUNIMAS_APOLLO_HOME", "data/apollo"),
            "ANNUNIMAS_PLUTUS_HOME": env_or("ANNUNIMAS_PLUTUS_HOME", "data/plutus"),
            "ANNUNIMAS_ORACLE_HOME": env_or("ANNUNIMAS_ORACLE_HOME", "data/oracle"),
            "ANNUNIMAS_WORLD_STATE_PATH": env_or("ANNUNIMAS_WORLD_STATE_PATH", "core/state/world.json"),
            "ANNUNIMAS_SOTERION_INDEX_PATH": env_or("ANNUNIMAS_SOTERION_INDEX_PATH", "data/soterion_index.json"),
            "ANNUNIMAS_TASK_QUEUE_PATH": env_or("ANNUNIMAS_TASK_QUEUE_PATH", "core/projects/tasks/queue.jsonl"),
            "ANNUNIMAS_PROJECT_TASK_QUEUE_PATH": env_or("ANNUNIMAS_PROJECT_TASK_QUEUE_PATH", "core/projects/tasks/queue.jsonl"),
            "ANNUNIMAS_DAILY_QUEUE_PATH": env_or("ANNUNIMAS_DAILY_QUEUE_PATH", "core/queue/queue.jsonl"),
            "ANNUNIMAS_WARDEN_QUEUE_PATH": env_or("ANNUNIMAS_WARDEN_QUEUE_PATH", "data/warden/informant_queue.jsonl"),
            "ANNUNIMAS_SYSTEM_CONTROL_PATH": env_or("ANNUNIMAS_SYSTEM_CONTROL_PATH", "core/state/system_control.json"),
        },
        "sockets": {
            "ANNUNIMAS_ATHENA_SOCKET": env_or("ANNUNIMAS_ATHENA_SOCKET", &default_runtime_socket("athena.sock")),
            "ANNUNIMAS_PROMETHEUS_SOCKET": env_or("ANNUNIMAS_PROMETHEUS_SOCKET", &default_runtime_socket("prometheus.sock")),
            "ANNUNIMAS_CHARON_SOCKET": env_or("ANNUNIMAS_CHARON_SOCKET", &default_runtime_socket("charon.sock")),
            "ANNUNIMAS_HADES_SOCKET": env_or("ANNUNIMAS_HADES_SOCKET", &default_runtime_socket("hades.sock")),
            "ANNUNIMAS_HERMES_SOCKET": env_or("ANNUNIMAS_HERMES_SOCKET", &default_runtime_socket("hermes.sock")),
            "ANNUNIMAS_MNEMOSYNE_SOCKET": env_or("ANNUNIMAS_MNEMOSYNE_SOCKET", &default_runtime_socket("mnemosyne.sock")),
            "ANNUNIMAS_APOLLO_SOCKET": env_or("ANNUNIMAS_APOLLO_SOCKET", &default_runtime_socket("apollo.sock")),
            "ANNUNIMAS_PLUTUS_SOCKET": env_or("ANNUNIMAS_PLUTUS_SOCKET", &default_runtime_socket("plutus.sock")),
            "ANNUNIMAS_ORACLE_SOCKET": env_or("ANNUNIMAS_ORACLE_SOCKET", &default_runtime_socket("oracle.sock")),
        },
        "models": {
            "ANNUNIMAS_LOCAL_MODEL": env_or("ANNUNIMAS_LOCAL_MODEL", "qwen2.5-coder:3b"),
        },
        "ruleset": ruleset
    })
}
...
pub(crate) fn format_tools_output(config: &Config, registered_agents: &[&str]) -> String {
    let mut lines = vec![
        "Annunimas v0.1.0 — Registered Agents".to_string(),
        "─────────────────────────────────────".to_string(),
    ];
...
fn prune_workspace_artifacts() -> serde_json::Value {
    ...
    let tmp_remove = if active_target.as_deref() == Some("/tmp/annunimas-target") {
        json!({
            "ok": false,
            "skipped": true,
            "reason": "active CARGO_TARGET_DIR is /tmp/annunimas-target"
        })
    } else {
        fs::remove_dir_all("/tmp/annunimas-target")
            .map(|_| json!({"ok": true}))
            .unwrap_or_else(|err| json!({"ok": false, "error": err.to_string()}))
    };
...
fn hardened_subsystem_inventory() -> Vec<(String, String, String)> {
    vec![
        ...
        (
            ...
            "crates/annunimas-oracle".to_string(),
        ),
        (
            "plutus".to_string(),
            "joulework".to_string(),
            "crates/annunimas-plutus".to_string(),
        ),
        (
            "apollo".to_string(),
            "workflow".to_string(),
            "crates/annunimas-apollo".to_string(),
        ),
    ]
}
...
fn emit_warden_informant_event(source: &str, issues: &[String]) -> serde_json::Value {
    ...
    if queue_path.trim().is_empty() {
        return json!({
            "ok": false,
            "error": "ANNUNIMAS_WARDEN_QUEUE_PATH is empty",
            "queue_path": queue_path,
        });
    }
    let event = json!({
...
fn personality_registry_path() -> std::path::PathBuf {
    std::env::var("ANNUNIMAS_AGENT_PERSONALITY_PATH")
...
mod tests {
    ...
    fn temp_root(name: &str) -> anyhow::Result<std::path::PathBuf> {
        ...
        Ok(std::env::temp_dir().join(format!("annunimas-cli-{name}-{stamp}")))
    }
