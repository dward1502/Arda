...
use crate::tools::{McpTool, McpToolGovernance};
...
pub fn playwright_bridge_contract() -> PlaywrightBridgeContract {
    let state_root = env::var("ANNUNIMAS_PLAYWRIGHT_STATE_DIR")
        .unwrap_or_else(|_| "data/mcp/playwright".to_string());
    PlaywrightBridgeContract {
        schema_version: "annunimas.mcp.playwright.v1".to_string(),
        runtime: "stdio_subprocess".to_string(),
        command: env::var("ANNUNIMAS_PLAYWRIGHT_MCP_CMD")
            .unwrap_or_else(|_| "npx @playwright/mcp@latest --stdio".to_string()),
        profile_dir: env::var("ANNUNIMAS_PLAYWRIGHT_PROFILE_DIR")
            .unwrap_or_else(|_| format!("{state_root}/profile")),
        artifact_dir: env::var("ANNUNIMAS_PLAYWRIGHT_ARTIFACT_DIR")
            .unwrap_or_else(|_| format!("{state_root}/artifacts")),
        log_path: env::var("ANNUNIMAS_PLAYWRIGHT_LOG_PATH")
            .unwrap_or_else(|_| format!("{state_root}/bridge.log")),
        pid_path: env::var("ANNUNIMAS_PLAYWRIGHT_PID_PATH")
