...
use crate::service::ApolloService;
use arda_core::error::Result;
use std::path::PathBuf;

#[derive(Debug, Clone)]
pub struct ApolloDaemonConfig {
    pub socket_path: PathBuf,
    pub http_enabled: bool,
    pub http_addr: String,
}

impl Default for ApolloDaemonConfig {
    fn default() -> Self {
        Self {
            socket_path: expand_home("data/apollo/apollo.sock"),
            http_enabled: true,
            http_addr: format!("{}:{}", "127.0.0.1", 5118),
        }
    }
}

pub struct ApolloDaemon {
    service: ApolloService,
    config: ApolloDaemonConfig,
}

impl ApolloDaemon {
    pub fn new(service: ApolloService, config: ApolloDaemonConfig) -> Self {
        Self { service, config }
    }
