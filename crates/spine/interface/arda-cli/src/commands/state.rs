// sigil: ∇ ◈ ↝
//
// `annunimas-cli state validate` — walks core/state/ and parses every
...
pub(crate) enum StateCommands {
    ...
    Validate {
        /// State root override (defaults to <ANNUNIMAS_ROOT>/core/state)
        #[arg(long)]
        root: Option<PathBuf>,
...
pub(crate) fn handle(cmd: StateCommands) -> anyhow::Result<()> {
    match cmd {
        StateCommands::Validate { root, verbose } => {
            let root = root.unwrap_or_else(|| annunimas_root().join("core/state"));
