//! Reusable backend projection, source loading, and interaction policy for Mirromere consumers.
...
impl MirromereInteractionReceiptState {
    pub fn remember_surface(&self, surface: MirromereSurfaceProjection) -> Result<(), String> {
...
pub async fn load_continuity_reference() -> ContinuityProjectionReference {
    let now = Utc::now();
