...
pub fn build_runtime_snapshot(root: &Path, now: DateTime<Utc>) -> ChronosRuntimeSnapshot {
    ...
    ChronosRuntimeSnapshot {
        schema_version: "arda.chronos-runtime.v1".to_string(),
        generated_at_utc: now,
        authority: "arda-chronos".to_string(),
    ...
    for task in due_tasks {
        ...
        if task.read_only {
            receipts.push(ChronosAuditTaskReceipt {
                schema_version: "arda.chronos-audit-receipt.v1".to_string(),
                authority: "arda-chronos".to_string(),
                generated_at_utc: now,
                task_id: task.id,
                task_name: task.name,
...
fn build_bounded_audit_runner_receipt(
    ...
    ChronosAuditRunnerReceipt {
        schema_version: "arda.chronos-bounded-audit-runner.v1".to_string(),
        authority: "arda-chronos".to_string(),
        generated_at_utc: now,
        task_id: task.id.clone(),
        audit_class: task.audit_class.clone(),
...
mod tests {
    ...
    fn runtime_snapshot_surfaces_typed_feeds_audit_runner_and_next_steps() {
        let root = std::env::temp_dir().join(format!(
            "arda_chronos_runtime_snapshot_test_{}",
        ...
        assert_eq!(snapshot.schema_version, "arda.chronos-runtime.v1");
    ...
    fn scheduled_audit_execution_writes_bounded_runner_receipts() {
        let root = std::env::temp_dir().join(format!(
            "arda_chronos_bounded_runner_test_{}",
            Utc::now().timestamp_nanos_opt().expect("timestamp nanos")
        ));
        let state_dir = root.join("core/state");
        let config_dir = root.join("config");
        ...
        assert_eq!(
            runner_receipt
                .get("schema_version")
                .and_then(serde_json::Value::as_str),
            Some("arda.chronos-bounded-audit-runner.v1")
        );
        assert_eq!(
            runner_receipt
                .get("mutation_policy")
