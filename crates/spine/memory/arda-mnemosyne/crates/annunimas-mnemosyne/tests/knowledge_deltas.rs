...
fn test_mnemosyne_memory_bridge_knowledge_deltas() {
    ...
    let knowledge_delta = InformantEvent {
        ...
        tags: vec!["knowledge".to_owned(), "delta".to_owned(), "test".to_owned()],
    };
    ...
    // Get the memory details (we can't directly check confidence_hint in RecallRecentEntry)
    ...
    println!("Knowledge delta successfully stored with metadata");
}
...
fn test_mnemosyne_memory_bridge_supersession() {
    ...
    let knowledge_delta_high = InformantEvent {
        ...
        confidence_hint: Some(0.95),
        tags: vec!["knowledge".to_owned(), "delta".to_owned(), "high".to_owned()],
    };
    ...
    // Verify that both memories are stored
    let identity = service.identity_state().expect("identity");
    assert!(identity.active_memory_count >= 1);
