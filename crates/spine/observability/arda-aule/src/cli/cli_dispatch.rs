...
pub(crate) async fn execute(cli: Cli, config: &Config) -> anyhow::Result<()> {
    match cli.command {
        ...
        Commands::Iterate {
            ...
            use arda_forge_mind::forge::{iterate_asset, IterateSpec};
            use arda_forge_mind::tools::comfyui::ComfyUiClient;
            use arda_forge_mind::tools::vision::VisionClient;

            let assets_root = assets_root
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(arda_forge_mind::forge::DEFAULT_ASSETS_ROOT));

            let render_config = arda_forge_mind::forge::RenderConfig {
                python: bpy_python
                    .map(PathBuf::from)
                    .unwrap_or_else(|| PathBuf::from(arda_forge_mind::forge::DEFAULT_PYTHON)),
                angles: angles
                    .map(|s| s.split(',').map(String::from).collect())
                    .unwrap_or_else(|| {
                        arda_forge_mind::forge::DEFAULT_ANGLES
                            .iter()
                            .map(|s| s.to_string())
                            .collect()
                    }),
            ...
            let spec = IterateSpec {
                ...
                material_family,
                overrides: arda_forge_mind::forge::GenerateOverrides::default(),
                budget_iters,
                accept_threshold,
                render_config,
            };
            ...
            let mut vision = VisionClient::new(
                vision_addr.unwrap_or_else(|| "http://arda-server:8081".to_string()),
...
async fn handle_run(config: &Config, task_type: &str, description: &str) -> anyhow::Result<()> {
    ...
    match &result.status {
        ...
        arda_core::TaskStatus::Pending => {
            println!("⏳ Task pending — execution pipeline active");
        }
        arda_core::TaskStatus::Failed { reason } => {
