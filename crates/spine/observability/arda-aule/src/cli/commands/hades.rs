...
fn queue_compaction_plan(
    ...
    let plan = json!({
        "schema_version": "ARDA.hades.queue-compaction-plan.v1",
        "plan_id": plan_id,
        "generated_at_utc": generated_at.to_rfc3339(),
        "authority": "arda-cli hades queue-compaction-plan",
    ...
    let approval = json!({
        "schema_version": "arda.hades.queue-compaction-approval.v1",
...
fn queue_compaction_apply(
    ...
    let receipt = json!({
        "schema_version": "arda.hades.queue-compaction-apply-receipt.v1",
        "generated_at_utc": generated_at.to_rfc3339(),
        "status": "applied",
...
fn audit_outcomes(root: &Path, limit: usize) -> anyhow::Result<serde_json::Value> {
    ...
    Ok(json!({
        "contract": "arda.hades.audit_outcomes.v1",
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        std::env::temp_dir().join(format!("arda-hades-{name}-{nanos}"))
    }
