...
use super::super::{arda_root, MetricsCommands};
    ...
    fn new() -> Result<Self> {
        Ok(Self {
            ...
            queue_raw_queued_rows_total: IntGauge::new(
                "arda_queue_raw_queued_rows_total",
            ...
            pressure_guard_oversize_files_total: IntGauge::new(
                "arda_pressure_guard_oversize_files_total",
            ...
            refresh_success: IntGauge::new(
                "arda_metrics_exporter_refresh_success",
                "1 if last refresh cycle completed without error",
            )?,
            refresh_last_unix: IntGauge::new(
                "arda_metrics_exporter_refresh_unix_seconds",
...
fn refresh(state: &ExporterState) {
    ...
    let pressure = root.join("data/prometheus/pressure_guard_last.json");
    ...
    set_audit_health(
        f,
        "pressure_guard",
        read_json(&root.join("data/prometheus/pressure_guard_last.json"))
            .as_ref()
...
async fn audit_health_handler(State(state): State<ExporterState>) -> Json<Value> {
    let root = &state.root;
    let pressure = read_json(&root.join("data/prometheus/pressure_guard_last.json"));
    ...
    Json(serde_json::json!({
        ...
        "paths": {
            "pressure_guard": "data/prometheus/pressure_guard_last.json",
...
fn build_state(system_metrics: bool) -> Result<ExporterState> {
...
pub(crate) async fn handle(command: MetricsCommands) -> Result<()> {
    match command {
        MetricsCommands::Serve {
            ...
            let state = build_state(system_metrics)?;
            refresh(&state);
        ...
        MetricsCommands::Snapshot { system_metrics } => {
            let state = build_state(system_metrics)?;
            refresh(&state);
            let encoder = TextEncoder::new();
            let mut buf = Vec::new();
