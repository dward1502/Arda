...
fn export_task_lifecycle_runtime() -> Result<Value> {
    ...
    let snapshot = json!({
        "schema_version": "arda.task-lifecycle-runtime.v1",
        "generated_at_utc": now_utc(),
...
fn export_l3_readiness_projection() -> Result<Value> {
    ...
    let payload = json!({
        ...
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export l3-readiness-projection",
        "projection_policy": {
            "read_only": true,
        ...
        "operator_surfaces": {
            ...
            "hermes": {
                "path": "crates/arda-hermes/src/service/runtime.rs",
...
fn export_project_intake_governance() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.project-intake-governance.v1",
        "generated_at_utc": now_utc(),
...
fn export_autonomy_task_truth() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.autonomy-task-truth.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export autonomy-task-truth",
...
fn write_queue_federation_projection(root: &Path) -> Result<Value> {
    ...
    let queue_sources = vec![
        ...
        queue_source_spec(
            ...
            "crates/arda-athena/core/projects/tasks/queue.jsonl",
            "promotion_candidate",
        ...
        queue_source_spec(
            ...
            "promotion_candidate",
            "requires_arda_arandur_queue_operation_v1",
            false,
        ),
        queue_source_spec(
            "human_workspace",
    ...
    let control_source = json!({
        "schema_version": "arda.queue-federation.v1",
    ...
    let payload = json!({
        "schema_version": "arda.queue-federation.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export queue-federation",
...
fn queue_federation_control_projection(queue_federation: &Value, flywheel: &Value) -> Value {
    ...
    json!({
        "schema_version": "arda.queue-federation-control.v1",
        "projection_cache": {
            "source_paths": [
...
fn athena_policy_promotion_candidates(
    ...
    let path = root.join("crates/arda-athena/core/projects/tasks/queue.jsonl");
    read_jsonl_objects(&path)
        .into_iter()
        ...
        .map(|record| {
            ...
            json!({
                "source_queue": "crates/arda-athena/core/projects/tasks/queue.jsonl",
    ...
    read_jsonl_objects(&path)
        ...
        .map(|record| {
            json!({
                ...
                "required_contract": "arda.arandur.queue_operation.v1",
...
fn export_moria_repository_contract() -> Result<Value> {
    ...
    let moria_root =
        env::var("ARDA_MORIA_ROOT").unwrap_or_else(|_| "/var/home/mythos/Moria".to_string());
    let payload = json!({
        "schema_version": "arda.moria-repository-contract.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export moria-repository-contract",
        ...
        "allowed_source_classes": [
            ...
            "arda_receipt_referencing_external_source"
        ],
        "gates": {
...
fn export_athena_active_learning_health() -> Result<Value> {
    ...
    let human_review_total = source_lane
        ...
        .count();
    let payload = json!({
        "schema_version": "arda.athena-active-learning-health.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export athena-active-learning-health",
        "contract": rel(&contract_path, &root),
...
fn export_hermes_compression_credential_gate() -> Result<Value> {
    ...
    let open_tasks = queue_summary
        ...
        .unwrap_or(0);
    let payload = json!({
        "schema_version": "arda.hermes-compression-credential-gate.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export hermes-compression-credential-gate",
        "contract": rel(&contract_path, &root),
...
fn write_queue_hygiene_projection(
    ...
    let raw_queued_total = raw_tasks
        ...
        .count();
    let payload = json!({
        "schema_version": "arda.queue-hygiene.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export queue-hygiene",
        "doctrine": {
...
fn export_governance_priority_runtime() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.governance-priority-runtime.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export governance-priority-runtime",
        ...
        "monitoring": {
            "prometheus_metrics": [
                "arda_queue_latest_open_total",
                "arda_queue_stale_raw_queued_rows_total",
                "arda_pressure_guard_status",
                "arda_pressure_guard_violations_total",
                "arda_audit_health_status"
            ],
...
fn export_edge_package_readiness() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.edge-package-readiness.v1",
        "generated_at_utc": now_utc(),
        "authority": "arda-cli export edge-package-readiness",
...
fn export_operator_legibility_contract() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.operator-legibility-contract.v1",
        "generated_at_utc": now_utc(),
        "authority": "external_absorption_brief + model_control_surface + runtime_governor",
        "mission": {
            "goal": "Make arda readable as a product and operator surface without creating a second authority plane.",
        ...
        "non_goals": [
            "Do not fork a second orchestration runtime outside arda state authority.",
...
fn export_governance_gap_backlog() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.governance-gap-backlog.v1",
        "generated_at_utc": now_utc(),
...
fn crate_summary(crate_dir: &Path, root: &Path) -> Value {
    ...
    for signal in SIGNALS {
        ...
        all_hits.sort();
        all_hits.dedup();
        let state = if crate_name == "arda-ceo" {
            "shim"
    ...
    json!({
        ...
        "kind": if crate_name == "arda-ceo" { "shim" } else { "workspace_crate" },
        "signals": signals,
    })
}
...
fn export_gate_matrix() -> Result<Value> {
    ...
    let payload = json!({
        "generated_at_utc": now_utc(),
        "authority": "governance_gate_matrix",
        "schema_version": "arda.governance.gate-matrix.v1",
...
fn export_plan_index() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.plan.index.v1",
        "generated_at_utc": now_utc(),
...
mod tests {
    ...
    fn temp_json_path(name: &str) -> Result<PathBuf> {
        ...
        Ok(std::env::temp_dir().join(format!("arda-cli-{name}-{stamp}.json")))
    }

    fn temp_root(name: &str) -> Result<PathBuf> {
        let stamp = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
        let root = std::env::temp_dir().join(format!("arda-cli-{name}-{stamp}"));
    ...
    fn queue_federation_control_projection_selects_only_receipt_valid_safe_local_candidates() {
        ...
        assert_eq!(
            ...
            Some("arda.queue-federation-control.v1")
        );
        assert_eq!(
