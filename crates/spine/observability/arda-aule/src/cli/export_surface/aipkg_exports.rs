...
pub(crate) fn export_aipkg_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.aipkg.contract.v1",
...
pub(crate) fn export_aipkg_edge_lab_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.aipkg-edge-lab-contract.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_aipkg_marketplace_separation_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.aipkg-marketplace-separation-contract.v1",
        "generated_at_utc": now_utc(),
        ...
        "source_alignment": {
            ...
            "arda_adaptation": pattern.get("arda_adaptation").cloned().unwrap_or(Value::Null),
        },
        "core_law": {
...
pub(crate) fn export_network_native_node_onboarding_contract_impl() -> Result<Value> {
    ...
    let pattern = openfang
        ...
        .unwrap_or_else(|| json!({}));
    let payload = json!({
        "schema_version": "arda.network-native-node-onboarding-contract.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_edge_identity_remediation_contract_impl() -> Result<Value> {
    ...
    let stale_clusters = fleet_recon
        ...
        .unwrap_or_default();
    let payload = json!({
        "schema_version": "arda.edge-identity-remediation-contract.v1",
        "generated_at_utc": now_utc(),
