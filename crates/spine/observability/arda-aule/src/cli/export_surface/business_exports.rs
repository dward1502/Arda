...
pub(crate) fn export_numenor_prime_merge_registry_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.numenor-prime-merge-registry.v1",
...
pub(crate) fn export_valinor_merge_registry_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.valinor-merge-registry.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_client_delivery_portfolio_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.client-delivery-portfolio.v1",
        "generated_at_utc": now_utc(),
        "authority": "core_clients_registry + business_client_controls",
        "mission": {
            "goal": "Separate client identity, delivery projects, and infrastructure ownership so arda can handle external work without collapsing it into the internal project queue."
        },
        "organization_model": {
            "layers": [
...
pub(crate) fn export_client_delivery_readiness_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.client-delivery-readiness.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_valinor_project_dossiers_impl() -> Result<Value> {
    ...
    let mut dossiers = vec![
        ...
        json!({
            ...
            "owner": "prometheus",
            "lane": "internal_product",
            "reason": "CRUSTIES now has both a canonical doc suite and an archived MVP pack, but no current arda execution plan has been raised yet.",
            "next_actions": [
                "Convert CRUSTIES memory into an arda-native product dossier and ship/no-ship decision gate.",
        ...
        json!({
            ...
            "reason": "This lineage is strategically important, but it should feed current ARDA/arda work as pattern memory, not as a direct backlog import.",
            "next_actions": [
                "Mine the refactor and CITADEL archive only when current ARDA or platform design work opens a bounded lane.",
                "Prefer modern arda runtime truth over older Valinor platform theory.",
            ],
    ...
    let payload = json!({
        "schema_version": "arda.valinor-project-dossiers.v1",
        "generated_at_utc": now_utc(),
        ...
        "status": "converted_to_arda_native_dossiers",
        "summary": {
        ...
        "conversion_rules": {
            ...
            "prefer_current_arda_runtime_truth": true,
...
pub(crate) fn export_imported_corpus_plan_portfolio_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.imported-corpus-plan-portfolio.v1",
        "generated_at_utc": now_utc(),
