...
pub(crate) fn export_crate_spawn_contract_impl() -> Result<Value> {
    ...
    let patterns = alignment
        ...
        .unwrap_or_else(|| json!({}));
    let payload = json!({
        "schema_version": "arda.crate-spawn-contract.v1",
        ...
        "scaffold_flow": {
            "command": "arda-cli utility create-crate-spawn-blueprint",
...
pub(crate) fn export_opencode_project_runtime_impl() -> Result<Value> {
    ...
    let runtime_state = json!({
        "schema_version": "arda.opencode-project-runtime.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_source_lesson_embodiment_registry_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.source-lesson-embodiment-registry.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_source_lesson_embodiment_backlog_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.source-lesson-embodiment-backlog.v1",
        "generated_at_utc": now_utc(),
...
fn source_artifacts(source_id: &str) -> Value {
    let mapping = match source_id {
        "src_df11630e" => vec![
            ...
            "arda-cli utility scrapling-fetch",
