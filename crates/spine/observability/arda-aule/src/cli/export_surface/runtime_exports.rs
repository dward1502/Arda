...
pub(crate) fn export_runtime_admission_receipts_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.runtime-admission-receipts.v1",
...
pub(crate) fn export_runtime_budget_policy_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": contract.and_then(|table| table.get("schema_version")).and_then(toml::Value::as_str).unwrap_or("arda.runtime-governor-budget.v1"),
        "generated_at_utc": now_utc(),
...
fn export_runtime_admission_pressure_snapshot(
    ...
    let payload = json!({
        "schema_version": "arda.runtime-admission-pressure.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_runtime_admission_recovery_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.runtime-admission-recovery.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_memory_governor_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.memory-governor.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_metrics_delta_impl() -> Result<Value> {
    ...
    if current_snapshot_id.is_none() || baseline_snapshot_id.is_none() {
        let payload = json!({
            "schema_version": "arda.metrics-delta.v1",
            "generated_at_utc": now_utc(),
...
pub(crate) fn export_edge_model_rollout_impl() -> Result<Value> {
    ...
    let targets = [
        EdgeRolloutTarget {
            ...
            log_path: "~/arda-model-logs/seed_bluefin.log",
        ...
        EdgeRolloutTarget {
            ...
            ssh_target: "ardaserver@100.118.123.88",
            log_path: "~/arda-model-logs/seed_backbone.log",
            runtime_kind: "ramalama",
            models: &[
        ...
        EdgeRolloutTarget {
            ...
            log_path: "~/arda-model-logs/seed_warden.log",
            runtime_kind: "ollama",
    ...
    let payload = json!({
        "schema_version": "arda.edge-model-rollout.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_runtime_governor_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.runtime-governor-contract.v1",
        "generated_at_utc": now_utc(),
...
pub(crate) fn export_search_runtime_contract_impl() -> Result<Value> {
    ...
    let payload = json!({
        "schema_version": "arda.search-runtime-contract.v1",
        "generated_at_utc": now_utc(),
        ...
        "package_posture": {
            ...
            "runtime_env_contract": [
                "ARDA_SEARCH_RUNTIME_URL",
                "ARDA_SEARCH_RUNTIME_HOST",
                "ARDA_SEARCH_RUNTIME_PORT",
                "ARDA_SEARCH_RUNTIME_IMAGE",
                "ARDA_SEARCH_RUNTIME_CONTAINER_NAME",
            ],
...
pub(crate) fn export_scrapling_runtime_contract_impl() -> Result<Value> {
    ...
    let configured_order = runtime_env
        .get("ARDA_ATHENA_CRAWL_PROVIDER")
        .cloned()
        .or_else(|| shared_env.get("ARDA_ATHENA_CRAWL_PROVIDER").cloned())
        .unwrap_or_else(|| "crawl4ai,scrapling".to_string());
    let crawl_profile = runtime_env
        .get("ARDA_ATHENA_CRAWL_PROFILE")
        .cloned()
        .or_else(|| shared_env.get("ARDA_ATHENA_CRAWL_PROFILE").cloned())
    ...
    let order = configured_order
        ...
        .map(ToOwned::to_owned)
        .collect::<Vec<_>>();
    let fetch_command = "arda-cli utility scrapling-fetch".to_string();
    let runtime_mode = runtime_env
        .get("ARDA_SCRAPLING_RUNTIME_MODE")
        .cloned()
        .or_else(|| shared_env.get("ARDA_SCRAPLING_RUNTIME_MODE").cloned())
    ...
    let payload = json!({
        "schema_version": "arda.scrapling-runtime-contract.v1",
        "generated_at_utc": now_utc(),
        ...
        "runtime_contract": {
            "entrypoint": {
                "kind": "cli_command",
                "command": "arda-cli utility scrapling-fetch",
            },
            "runtime_probe": "scripts/runtime/scrapling_runtime.sh",
            "required_env": [
                "ARDA_ATHENA_CRAWL_PROVIDER",
                "ARDA_ATHENA_CRAWL_PROFILE",
                "ARDA_SCRAPLING_RUNTIME_MODE",
            ],
            "fallback_env": ["ARDA_CRAWL4AI_URL"],
        ...
        "promotion_gates": [
            {
                ...
                "evidence": "arda-cli utility scrapling-fetch + scripts/runtime/scrapling_runtime.sh",
            },
            {
