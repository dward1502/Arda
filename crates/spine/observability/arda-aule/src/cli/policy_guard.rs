...
fn command_policy_descriptor(command: &Commands) -> CommandPolicyDescriptor {
    match command {
        Commands::Hades {
            ...
            signature: "cargo run -p arda-cli -- hades remove".to_string(),
            is_network: false,
            is_destructive: true,
        ...
        Commands::Hermes {
            ...
            signature: "cargo run -p arda-cli -- hermes send".to_string(),
            is_network: true,
        ...
        Commands::Hermes {
            ...
            signature: "cargo run -p arda-cli -- hermes network-op".to_string(),
            is_network: true,
            is_destructive: false,
            decision_class: "routine_maintenance",
            human_authorizers: vec![],
        ...
        Commands::Control {
            ...
            signature: "cargo run -p arda-cli -- control route-governor".to_string(),
            is_network: false,
            is_destructive: false,
            decision_class: "provider_reroute",
            human_authorizers: vec![],
        ...
        Commands::Athena {
            ...
            signature: "cargo run -p arda-cli -- athena policy-promote".to_string(),
            is_network: false,
            is_destructive: false,
        ...
        Commands::Utility {
            ...
            signature: "cargo run -p arda-cli -- utility task-pivot".to_string(),
            is_network: false,
            is_destructive: false,
            decision_class: "routine_maintenance",
            human_authorizers: vec![],
        ...
        _ => CommandPolicyDescriptor {
            signature: "cargo run -p arda-cli -- local-op".to_string(),
            is_network: false,
            is_destructive: false,
            decision_class: "routine_maintenance",
            human_authorizers: vec![],
...
fn persist_policy_guard_record(record: &serde_json::Value) {
    let path = crate::arda_root().join("data/prometheus/policy_guard.jsonl");
...
fn emit_policy_guard_escalation(command: &str, reason: &str) {
    ...
    let path = crate::arda_root().join("data/prometheus/escalations.jsonl");
    if let Some(parent) = path.parent() {
...
fn permission_profile_denial_reason(
    ...
    let profile_id = std::env::var("ARDA_PERMISSION_PROFILE")
        .ok()
        .or_else(|| {
            state
                .get("active_profile")
...
fn persist_permission_profile_decision(record: &serde_json::Value) {
    let path = crate::arda_root().join("data/warden/permission_profile_audit.jsonl");
    if let Some(parent) = path.parent() {
        if let Err(err) = fs::create_dir_all(parent) {
            tracing::warn!(
                error = %err,
...
fn default_ruleset_state() -> serde_json::Value {
    serde_json::json!({
        ...
        "expires_at_utc": null,
        "policy": ruleset_policy("arda_totality")
    })
}
...
fn validator_contract(profile: &str) -> serde_json::Value {
    match profile {
        "citadel_business" => serde_json::json!({
            "schema_version": "arda.validators.v2",
            "core": {
                "joulework": {
                    "validator": "joulework",
                    "version": "v1.2.0",
    ...
    match profile {
        "citadel_business" => serde_json::json!({
            ...
            "network_unlock_templates": [
                "git push",
                "cargo run -p arda-cli -- hermes send"
            ]
        }),
        _ => serde_json::json!({
            "profile": "arda_totality",
            "gate_strict": false,
            ...
            "validators": validator_contract("arda_totality"),
            "human_augmentation": human_augmentation_policy("arda_totality"),
            "network_unlock_templates": [
                "git push",
                "cargo run -p arda-cli -- hermes send"
            ]
...
fn normalize_ruleset_profile(profile: &str) -> Option<&'static str> {
    ...
    match normalized.as_str() {
        "arda_totality" | "arda" | "totality" => Some("arda_totality"),
...
pub(crate) fn set_active_ruleset(
    ...
    let Some(profile) = normalize_ruleset_profile(profile) else {
        anyhow::bail!(
            "invalid ruleset profile '{}'; use arda_totality or citadel_business",
            profile
        );
    };
...
mod tests {
    ...
    fn human_augmentation_denial_reason_requires_triad_approval_for_provider_reroute() {
        ...
        let policy = ruleset_policy("arda_totality");
    ...
    fn read_only_system_control_load_does_not_create_or_refresh_state_file() {
        let nanos = std::time::SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("arda-system-control-readonly-{nanos}"));
        let path = dir.join("system_control.json");
        std::fs::create_dir_all(&dir).expect("temp dir");

        let state = read_system_control_state_from_path(&path);

        assert_eq!(state["schema_version"], "arda.system.control.v1");
        assert!(
            !path.exists(),
    ...
    fn governance_policy_mode_supports_observe_only_and_block_on_fail_overrides() {
        let descriptor = CommandPolicyDescriptor {
            signature: "cargo run -p arda-cli -- audit readonly".to_string(),
            is_network: false,
            is_destructive: false,
    ...
    fn governance_policy_mode_requires_independent_receipts_for_destructive_actions() {
        ...
        let approval_state = json!({
            "approvals": [{
                "status": "approved",
                "decision_class": "destructive_delete",
                "command_signature": "cargo run -p arda-cli -- hades remove",
                "approvers": ["arandur"],
