...
#[test]
fn metrics_serve_help_defaults_to_loopback() {
    let output = Command::new(env!("CARGO_BIN_EXE_arda-cli"))
        .args(["metrics", "serve", "--help"])
        .output()
        .expect("run arda metrics serve help");
    ...
    assert!(stdout.contains("[default: 127.0.0.1]"), "stdout: {stdout}");
}
