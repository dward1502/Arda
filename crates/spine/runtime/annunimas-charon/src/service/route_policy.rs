...
pub(super) fn build_route_decision_with_governance_chain(
    ...
    policy: &HybridRoutePolicy,
    route_profile: &RouteExecutionProfile,
    route_task: &annunimas_core::Task,
...
fn lane_fitness_half_life_hours() -> f64 {
    std::env::var("ANNUNIMAS_CHARON_LANE_FITNESS_HALF_LIFE_HOURS")
        .ok()
...
fn lane_fitness_prune_hours() -> f64 {
    std::env::var("ANNUNIMAS_CHARON_LANE_FITNESS_PRUNE_HOURS")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .filter(|value| *value > 0.0)
        .unwrap_or(72.0)
...
fn cooldown_bypass_max_seconds() -> u64 {
    std::env::var("ANNUNIMAS_CHARON_COOLDOWN_BYPASS_MAX_SECONDS")
        .ok()
...
fn half_open_probe_stride() -> u32 {
    std::env::var("ANNUNIMAS_CHARON_HALF_OPEN_PROBE_STRIDE")
        .ok()
...
pub(super) fn tool_execution_min_context_window() -> usize {
    std::env::var("ANNUNIMAS_CHARON_TOOL_EXECUTION_MIN_CONTEXT")
        .ok()
...
fn compression_min_context_window() -> usize {
    std::env::var("ANNUNIMAS_CHARON_COMPRESSION_MIN_CONTEXT")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|value| *value >= 16_000)
        .unwrap_or(96_000)
...
fn local_tool_context_headroom_tokens() -> usize {
    std::env::var("ANNUNIMAS_CHARON_LOCAL_TOOL_CONTEXT_HEADROOM")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
...
fn configured_tool_incompatible_model(provider_id: &str, model_id: &str) -> bool {
    std::env::var("ANNUNIMAS_CHARON_TOOL_INCOMPATIBLE_MODELS")
        .unwrap_or_default()
        .split(',')
        .map(str::trim)
...
pub(super) fn resolve_hybrid_route_policy(
    ...
    let env_privacy =
        std::env::var("ANNUNIMAS_ROUTE_PRIVACY_DEFAULT").unwrap_or_else(|_| "public".to_string());
    let env_cost =
        std::env::var("ANNUNIMAS_ROUTE_COST_DEFAULT").unwrap_or_else(|_| "balanced".to_string());
    let env_quality =
        std::env::var("ANNUNIMAS_ROUTE_QUALITY_DEFAULT").unwrap_or_else(|_| "balanced".to_string());
    let env_origin =
        std::env::var("ANNUNIMAS_ROUTE_ORIGIN_DEFAULT").unwrap_or_else(|_| "auto".to_string());
    let env_latency = std::env::var("ANNUNIMAS_ROUTE_LATENCY_SLA_MS")
        .ok()
