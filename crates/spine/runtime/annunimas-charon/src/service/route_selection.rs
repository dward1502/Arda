...
fn fallback_tier_order() -> Vec<String> {
    std::env::var("ANNUNIMAS_CHARON_FALLBACK_TIER_ORDER")
...
fn large_context_tool_execution_threshold() -> usize {
    std::env::var("ANNUNIMAS_CHARON_LARGE_TOOL_CONTEXT_THRESHOLD")
        .ok()
...
fn free_external_tool_pool_min_candidates() -> usize {
    std::env::var("ANNUNIMAS_CHARON_FREE_TOOL_POOL_MIN_CANDIDATES")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
...
fn configured_free_pool_provider(provider_id: &str) -> bool {
    let configured = std::env::var("ANNUNIMAS_CHARON_FREE_POOL_PROVIDER_IDS")
...
mod tests {
    ...
    fn free_pool_provider_ids_are_env_configurable() {
        ...
        std::env::set_var(
            "ANNUNIMAS_CHARON_FREE_POOL_PROVIDER_IDS",
            "openrouter,google",
        );
        ...
        std::env::remove_var("ANNUNIMAS_CHARON_FREE_POOL_PROVIDER_IDS");
    }
    ...
    fn mixed_tier_provider_can_enter_free_pool_by_configured_id() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        std::env::set_var("ANNUNIMAS_CHARON_FREE_POOL_PROVIDER_IDS", "google");
    ...
    fn free_tool_pool_does_not_collapse_to_tiny_free_set() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        std::env::remove_var("ANNUNIMAS_CHARON_FREE_TOOL_POOL_MIN_CANDIDATES");
    ...
    fn explicit_fallback_tier_order_keeps_local_before_paid_cloud_for_planning() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        std::env::remove_var("ANNUNIMAS_CHARON_FALLBACK_TIER_ORDER");
    ...
    fn free_tool_pool_ignores_near_exhausted_free_providers() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        std::env::set_var("ANNUNIMAS_CHARON_FREE_TOOL_POOL_MIN_CANDIDATES", "2");
