...
fn stale_reservation_ttl_seconds() -> i64 {
    std::env::var("ANNUNIMAS_CHARON_RESERVATION_TTL_SECONDS")
        .ok()
...
pub(super) fn provider_unavailable_reason(
    ...
    if super::provider_in_half_open(p) {
        return Some(serde_json::json!({
            ...
            "probe_stride": std::env::var("ANNUNIMAS_CHARON_HALF_OPEN_PROBE_STRIDE")
                .ok()
...
impl CharonService {
    pub(super) async fn refresh_local_provider_health(&self) -> Result<()> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|err| annunimas_core::error::AnnunimasError::Agent {
...
fn local_health_probe_interval_seconds() -> i64 {
    std::env::var("ANNUNIMAS_CHARON_LOCAL_HEALTH_PROBE_INTERVAL_SECONDS")
        .ok()
        .and_then(|value| value.parse::<i64>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(45)
...
fn local_health_probe_timeout_seconds() -> u64 {
    std::env::var("ANNUNIMAS_CHARON_LOCAL_HEALTH_PROBE_TIMEOUT_SECONDS")
        .ok()
...
fn local_model_probe_timeout_seconds() -> u64 {
    std::env::var("ANNUNIMAS_CHARON_LOCAL_MODEL_PROBE_TIMEOUT_SECONDS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(6)
...
mod tests {
    ...
    fn stale_reservation_ttl_uses_positive_env_override() {
        ...
        std::env::set_var("ANNUNIMAS_CHARON_RESERVATION_TTL_SECONDS", "30");
        assert_eq!(stale_reservation_ttl_seconds(), 30);
        std::env::set_var("ANNUNIMAS_CHARON_RESERVATION_TTL_SECONDS", "0");
        assert_eq!(stale_reservation_ttl_seconds(), 900);
        std::env::remove_var("ANNUNIMAS_CHARON_RESERVATION_TTL_SECONDS");
    }
}
