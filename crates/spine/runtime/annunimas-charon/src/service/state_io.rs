...
pub(crate) fn runtime_build_cache_state_path() -> PathBuf {
    std::env::var("ANNUNIMAS_RUNTIME_BUILD_CACHE_STATE_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            super::paths::annunimas_root().join("core/state/runtime_build_cache.json")
        })
}
...
pub(crate) fn default_root() -> PathBuf {
    ...
    super::paths::annunimas_root().join("data/charon")
}

pub(crate) fn is_permission_error(err: &AnnunimasError) -> bool {
    matches!(
        err,
