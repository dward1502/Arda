...
use crate::service::runtime_state::{failure_backoff_seconds, merge_runtime_state};
use crate::service::state_io::append_jsonl;
use crate::types::RouteDecision;
use annunimas_core::error::Result;
use annunimas_core::machine_sigil_or_default;
...
impl CharonService {
    ...
    pub(crate) fn record_tool_fit_observation(
        ...
        req: &crate::types::CharonRequestEnvelope,
    ...
    fn record_provider_capability_receipts(&self, observation: &ToolFitObservation) -> Result<()> {
        ...
        if let Err(err) = unlock_result {
            return Err(annunimas_core::error::AnnunimasError::Ledger(err));
        }
...
fn positive_receipt_ttl_hours() -> i64 {
    std::env::var("ANNUNIMAS_CHARON_CAPABILITY_POSITIVE_TTL_HOURS")
        .ok()
...
fn negative_receipt_ttl_hours() -> i64 {
    std::env::var("ANNUNIMAS_CHARON_CAPABILITY_NEGATIVE_TTL_HOURS")
        .ok()
        .and_then(|value| value.parse::<i64>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(24)
