...
use arda_governance::{calculate_resonance_basic, triad_validate, TriadConfig};
use arda_plutus::{JouleWorkUnit, LoveEquation, PlutusService};
use chrono::Utc;
...
impl HadesService {
    ...
    pub(super) fn notify_warden(&self, event: &str, file: &Path) -> Result<serde_json::Value> {
        ...
        let global_path = std::env::var("Ardad_WARDEN_QUEUE_PATH")
            .map(PathBuf::from)
    ...
    pub(super) fn handoff_athena(&self, event: &str, file: &Path) -> serde_json::Value {
        let file_display = file.display().to_string();
        let socket_path = std::env::var("Ardad_ATHENA_SOCKET")
            .map(PathBuf::from)
    ...
    fn send_athena_ipc(
        ...
        let mut stream = UnixStream::connect(socket_path).map_err(|e| ArdadError::Agent {
