...
const LIFECYCLE_AUDIT_CONTRACT: &str = "arda.hades.lifecycle_audit_report.v1";
const LIFECYCLE_FINDING_CONTRACT: &str = "arda.hades.lifecycle_finding.v1";
const LIFECYCLE_REVIEW_QUEUE_CONTRACT: &str =
    "arda.hades.lifecycle_review_queue_projection.v1";
const LIFECYCLE_APPROVAL_PACKET_CONTRACT: &str =
    "arda.hades.lifecycle_operator_approval_packet.v1";
const LIFECYCLE_CLEANUP_CONTRACT: &str = "arda.hades.lifecycle_cleanup_executor.v1";
...
impl HadesService {
    ...
    pub fn warden_hades_signed_approval_packet(
        ...
        let review_items = review_packet
            ...
            .ok_or_else(|| {
                ArdaError::Task("source review packet has no review_items array".to_owned())
            })?;
    ...
    pub fn warden_hades_dry_run_receipt(
        ...
        let selected_review_ids = selected_review_ids
            ...
            .map(|id| {
                id.as_str().map(str::to_owned).ok_or_else(|| {
                    ArdaError::Task("selected_review_ids entries must be strings".to_owned())
                })
            })
        ...
        let selected_items = approval_packet
            ...
            .cloned()
            .ok_or_else(|| {
                ArdaError::Task("approval packet must include selected_items array".to_owned())
            })?;
    ...
    pub fn warden_hades_signed_mutation_approval_packet(
        ...
        let selected_items = dry_run_receipt
            ...
            .cloned()
            .ok_or_else(|| {
                ArdaError::Task("dry-run receipt must include selected_items array".to_owned())
            })?;
    ...
    pub fn warden_hades_final_apply_execution(
        ...
        for item in selected_items {
            let archived_item = serde_json::json!({
                "contract": "Arda.hades.warden_operator_archived_review_record.v1",
...
fn write_review_evidence_artifact(
    ...
    let artifact = WardenHadesEvidenceArtifactRecord {
        contract: "arda.hades.warden_queue_evidence_artifact.v1".to_owned(),
        generated_at_utc: Utc::now().to_rfc3339(),
