...
/// Minimum admissible context window for any route. Defaults to 16_384 to keep
/// legacy providers (which assume >=16K context) viable. Lowered (e.g. 8192) on
/// nodes that serve hardware-capped small-context models like Ternary-Bonsai-27B
/// (ctx capped at 8192 by 2x8GB VRAM). Does NOT relax the tool/execution guards
/// (tool_execution_min_context_window / execution-lane >=32K) which exist to stop
/// Hermes tool-call crashes on undersized context.
pub(super) fn min_context_floor() -> usize {
    std::env::var("ARDA_MANWE_MIN_CONTEXT_WINDOW")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
...
fn round_up_context_tier(tokens: usize) -> usize {
    let floor = min_context_floor();
    let bumped = if tokens < floor { floor } else { tokens };
    match bumped {
        // When the node's context floor is below 16K (e.g. 8192 for hardware-capped
        // models), the bottom tier rounds up to the floor instead of 16K.
        n if n <= floor => floor,
        0..=16_000 => 16_000,
...
pub(super) fn derive_route_execution_profile(
    ...
    if matches!(priority, "background" | "low" | "deferred") {
        return RouteExecutionProfile {
            route_class: "background_maintenance".to_string(),
            execution_lane: "background".to_string(),
            context_window_target: min_context_floor(),
        };
    }
