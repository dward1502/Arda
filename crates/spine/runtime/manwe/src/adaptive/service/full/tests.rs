...
async fn half_open_route_preserves_failure_streak_until_confirmed_success() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ARDA_MANWE_HALF_OPEN_PROBE_STRIDE", "1");
    let dir = tempdir().expect("tempdir");
    ...
    let svc = CharonService::new(dir.path()).expect("service");
    {
        ...
        provider.models[0].context_window = 65_536;
    }

    // Exercise half-open admission independently of execution-lane context and
    ...
    let provider = providers
        ...
        .find(|provider| provider.id == "local_fallback")
        .expect("local_fallback provider");
    std::env::remove_var("ARDA_MANWE_HALF_OPEN_PROBE_STRIDE");
    drop(_guard);
...
async fn per_agent_quota_blocks_only_exhausted_agent() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ARDA_MANWE_AGENT_REQUESTS_PER_DAY", "1");
    let dir = tempdir().expect("tempdir");
    isolate_test_provider_config(dir.path());
    let svc = CharonService::new(dir.path()).expect("service");
    let req_for = |agent_id: &str| ManweRequestEnvelope {
    ...
    std::env::remove_var("ARDA_MANWE_AGENT_REQUESTS_PER_DAY");
    drop(_guard);
...
async fn bandit_records_provider_result_and_affects_score() {
    let _guard = isolate_env_lock(&ENV_LOCK);
    std::env::set_var("ARDA_MANWE_BANDIT_MIN_OBSERVATIONS", "1");
    std::env::set_var("ARDA_MANWE_BANDIT_SCORE_WEIGHT", "8");
    let dir = tempdir().expect("tempdir");
    let svc = CharonService::new(dir.path()).expect("service");
    ...
    let plain_req = ManweRequestEnvelope {
        agent_id: "athena".to_string(),
        task_type: "chat".to_string(),
        priority: "normal".to_string(),
        messages: vec![serde_json::json!({"role":"user","content":"bandit test"})],
    ...
    std::env::remove_var("ARDA_MANWE_BANDIT_MIN_OBSERVATIONS");
    std::env::remove_var("ARDA_MANWE_BANDIT_SCORE_WEIGHT");
    drop(_guard);
