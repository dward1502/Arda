...
impl CharonService {
    ...
    pub async fn route_observability_rollup(&self) -> Result<JsonValue> {
        let providers = self.providers().await;
        let recent_events = self.recent_state_events(1_000).await;
...
fn provider_is_free_pool_member(
    provider: &crate::adaptive::service::types::ProviderState,
...
fn model_is_free(model: &crate::adaptive::service::types::ModelState) -> bool {
