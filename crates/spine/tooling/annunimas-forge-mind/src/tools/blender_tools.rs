...
//!
//! Each tool exposes [`annunimas_tool_harness::types::ToolMetadata`] so that
...
use annunimas_tool_harness::service::build_invocation_plan;
...
impl BlenderToolRegistry {
    ...
    pub async fn invoke(
        ...
        if matches!(
            plan.disposition,
            annunimas_tool_harness::types::InvocationDisposition::HoldForOperatorReview
        ) {
...
fn tool_meta(tool_id: &str, risk: RiskLevel, side_effect: SideEffectClass) -> ToolMetadata {
    ToolMetadata {
        ...
        input_schema_ref: format!("annunimas.tool.{tool_id}.input.v1"),
        output_schema_ref: format!("annunimas.tool.{tool_id}.output.v1"),
        risk_level: risk,
...
mod tests {
    use super::*;
    use annunimas_tool_harness::types::InvocationDisposition;
    use serde_json::json;
