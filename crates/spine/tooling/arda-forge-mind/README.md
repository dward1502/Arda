...
Spawned from the Arda sovereign crate blueprint. Workflow + execution
surface for 3D asset production (Blender, texturing, slicing, ARDA scenes).

- Realm: `operations`
- Productizable: `true`
- Role: blueprint workflow + execution surface
- Required exports: `core/state/arda-forge-mind.json`
...
```rust
use arda_forge_mind::service::workflow_plan;
use arda_forge_mind::workflow::{ArtifactPolicy, EngineeringDomain, ForgeWorkItem};
...
- `blender_tools.rs` — `BlenderToolRegistry` that pairs each command with an
  `arda-tool-harness` `ToolMetadata` so every call passes through the
...
```bash
cargo build -p arda-forge-mind --features mcp-bridge
# or both at once
cargo build -p arda-forge-mind --features "native-blender mcp-bridge"
```

The bridge has no extra dependencies beyond what forge-mind already pulls in
(tokio, serde, anyhow, arda-tool-harness).

### Calling a tool

```rust
use arda_forge_mind::tools::blender_tools::{fresh_envelope, BlenderToolRegistry};
...
The workstation / `arda-server` is required for the full build flow:

- ComfyUI + Hunyuan3D-2 text-to-3D generation at `http://arda-server:8188`
- vision feedback endpoint at `http://arda-server:8081`
...
6. From the arda repo, check Forge-Mind endpoint resolution:

   ```bash
   source scripts/runtime_build_env.sh
   cargo run -p arda-cli -- forge status
   ```
...
```bash
source scripts/runtime_build_env.sh
cargo run -p arda-cli -- forge generate \
  ...
  --comfyui-addr http://arda-server:8188 \
  --hy3d-octree 384 \
  --post-cleanup
```
...
cargo run -p arda-cli -- iterate \
  ...
  --material-family boardroom_monitor_bezel \
  --comfyui-addr http://arda-server:8188 \
  --vision-addr http://arda-server:8081 \
  --budget-iters 3 \
...
```bash
source scripts/runtime_build_env.sh
cargo run -p arda-cli -- forge upgrade upper_monitor_1 \
  --domain world \
  --template prompt_3 \
  --scene-binding upper_monitor_1 \
  --material-family boardroom_monitor_bezel
