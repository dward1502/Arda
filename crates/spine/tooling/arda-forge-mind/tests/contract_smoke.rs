use arda_forge_mind::contract::{contract, ArtifactPolicy};
use arda_forge_mind::service::{
    next_stage, status, validate_governance_baseline, workflow_plan,
};
use arda_forge_mind::workflow::{EngineeringDomain, ForgeWorkItem, WorkflowStage};
...
fn sovereign_baseline_contract_is_present() {
    ...
    assert_eq!(
        ...
        "core/state/arda-forge-mind.json"
    );
