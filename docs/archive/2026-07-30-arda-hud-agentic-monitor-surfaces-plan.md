...
## Current baseline
- `apps/arda-hud/src/scene/boardroom/boardroomSpatialLayout.ts` defines 5 upper-monitor zones, 4 lower desk surfaces, 1 control core, and physical controls. Four monitor zones expose assignable slot IDs (`monitor_left_1..monitor_left_4`); the center zone remains a non-assignable scene anchor.
...
Slices A–E are implemented and the native interaction gate is complete. The completed
runtime includes persisted claims, frontend claim synchronization, real
`monitor-surface-payload` event consumption, native create/focus/dismiss calls,
lease refresh, a stateful Rust ownership registry, bounded Hermes owner and
payload namespaces, and exact owner/binding authorization for updates.

The final operator-visible walkthrough ran in the native Tauri/WebKit application
with the development-only monitor acceptance surface enabled. The walkthrough
recorded 7/7 successful checks: claim, authorized payload delivery and visible
monitor rendering, native create/focus, lease refresh, reload fallback followed by
post-reload payload delivery, unauthorized-owner rejection, and release/dismiss.
The child monitor window was present after create/focus and absent after dismiss;
the claimed upper monitor returned to its operator-configured fallback after
release.

The native walkthrough exposed and closed two integration defects that unit tests
had not covered: scene zone IDs were being sent to the monitor contract instead of
stable assignment slot IDs, and active claims were rendered only when the
operator-configured focus mode was `remote_preview`. The renderer now resolves the
contract slot from `assignmentSlotId` and presents every active claim while keeping
the configured focus mode responsible only for activation behavior. Focused RED/GREEN
tests cover both corrections.

This plan is complete and retained under `docs/archive/`; no remaining implementation
or acceptance item belongs to this plan.
