...
> Manwe: 📜 inference gateway | owner: manwe | status: archived | reviewed: 2026-07-27

# Manwe Foundation Plan — Archived

## Name / Identity

`CHARON` is implemented as `manwe` in `crates/spine/runtime/manwe`. This
completed foundation plan was archived on 2026-07-27. Current operator and
maintenance truth lives in the crate's `README.md`, `STATUS.md`,
`BREAKDOWN.md`, and `PROVIDERS.md`; timestamped runtime evidence below is
retained as history rather than current provider availability.
...
- The former annunimas-server coder, vision, and LFM services are intentionally
  ...
  Manwe must keep it unroutable until its normal health probe succeeds. At the
  time of this evidence, the repaired Beelink Carnice lane was active on
  `:1234`; its current enrollment is intentionally determined by live config.
...
6. Operator documentation completed 2026-07-27.
   - Current contracts are maintained with the live crate rather than in this
     archived plan.
   - Runtime posture remains evidence-based and timestamp-sensitive.
