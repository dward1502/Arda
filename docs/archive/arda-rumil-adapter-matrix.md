...
**Status:** Archived planning inventory; live adapter authority is the `arda-rumil` crate.

This file preserves the reuse-first adapter inventory that guided the initial implementation. Current adapter availability, feature gates, and profile behavior are authoritative in the live `arda-rumil` crate.
