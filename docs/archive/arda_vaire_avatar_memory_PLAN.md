...
**Status:** Complete and archived 2026-08-03. Identity derivation, promotion,
decay, local Markdown projection, and the HUD consumer are implemented and
verified; broader personal-ops, business, and curiosity work remains explicitly
outside this plan.
...
**File:** `crates/spine/memory/arda-vaire/src/persona/types.rs`
...
Source memories contribute only explicit typed evidence from
`extensions["persona.trait_evidence"]`; arbitrary tags and prose are never
promoted into identity claims.

---

## 2. Identity derivation

**Files:** `crates/spine/memory/arda-vaire/src/persona/derive.rs` and
`crates/spine/memory/arda-vaire/src/service/persona_derive.rs`

Function: `derive_identity_summary(actor: &str, since: Option<DateTime<Utc>>) -> Result<PersonaProjection>`

- Reads eligible canonical episodic `MemoryRecord`s where `record.agent == "arandur"` from the existing contract-memory root; no new index.
...
- [x] `derive_identity_summary` runs as part of existing consolidate pass with zero new background jobs.
- [x] Running it twice on the same input set is idempotent (same trait confidence output, atomically replaced rather than append-only duplication).
- [x] Decayed/revoked records are provably excluded by a focused revoked-record regression.
...
**Acceptance criteria:**
- [x] Unit test: single evidence record does not appear in `persona.traits` output.
- [x] Unit test: 3 records within window does appear, confidence = 0.3.
- [x] Unit test: trait untouched for 61 days gets `stale: true` on next derivation pass, is not removed from the array.
...
**Acceptance criteria:**
- [x] A mood sample from 5 days ago has materially less influence than one from 5 hours ago (test asserts the exact weighted result and a greater-than-20× recency ratio).
- [x] Empty mood window returns `None`, not divide-by-zero.

---

## 5. Obsidian projection

**Pathway:** the existing consolidation pass writes an atomic local Markdown
projection. The existing `sync_obsidian` method remains optional note-import
tooling and is not reused as an output or account-integration channel.

- Output: `human/personality/arandur/<date>.md`
- `human/` is a local filesystem projection tree that may be opened as an
  Obsidian vault. No Obsidian account, credential, plugin, or cloud API is
  required or configured by Vairë.
...
- Regenerated directly by the existing Vairë consolidation pass; no separate Obsidian writer or sync job is introduced.

**Acceptance criteria:**
- [x] File is regenerated atomically (not appended) each consolidation cycle.
- [x] Stale traits render with both strikethrough and a `(stale)` suffix.

---

## 6. HUD consumer

**File:** `apps/arda-hud/src/lib/statefulPersona.ts` (state consumer alongside the unchanged `avatarPersona.ts` rendering-skin helpers)
...
**Acceptance criteria:**
- [x] `statefulPersona.ts` has zero hardcoded persona state; all trait/mood values trace to a `persona.*` read.
- [x] Subpanel renders with zero data gracefully (new install, no evidence yet) — shows an empty/neutral state, not an error.
- [x] No new sync pipeline, no new store, no new IPC channel — confirmed by diff review before merge.
