...
//!
//! The registry validates `ServiceContract`s and exposes idempotent mutation
//! APIs so the engine can manage a current view of running services without
//! carrying the full persistence burden during bootstrap.

use std::collections::HashMap;

use crate::contract::{ServiceContract, ServiceKind};
use crate::service::{ServiceHandle, ServiceRecord, ServiceStatus};
...
impl ServiceRegistry {
    ...
    pub fn mark_started(&mut self, name: &str, handle: ServiceHandle) -> Result<(), RegistryError> {
        let entry = self.records.get_mut(name).ok_or_else(|| RegistryError::NotFound(name.to_string()))?;
    ...
    pub fn by_kind(&self, kind: ServiceKind) -> Vec<&ServiceRecord> {
        self.records.values().filter(|record| record.contract.kind == kind).collect()
    }
}
