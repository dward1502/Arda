...
- [x] arda-economics  ← absorbs arda-plutus boundary
  - Ported plutus economy/JW modules into `crates/spine/runtime/arda-economics`.
  - Migrated all `arda_plutus::*` consumers in `arda-mandos`, `arda-vaire`, `arda-varda`, `arda-orome` to `arda_economics::*`.
  - `cargo check -p arda-economics -p arda-mandos -p arda-vaire -p arda-varda -p arda-orome` green.
  - Removed `arda-plutus` workspace member; retired crate.
- [x] arda-mandos  → lib ✓
...
- [x] arda-service-registry → lib / thin
  ...
  - `engine` uses it as the service metadata facade (`pub use arda_service_registry as service_registry`); the launcher backend owns the onboarding flow now.
...
- [x] arda-human → folded into `arda-varda`
  - Human-knowledge surface is now under `arda-varda`'s ingest/human module.
  - `arda-human` is no longer a separate workspace member.
