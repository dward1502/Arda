...
| Workbench project validation and attachment | `arda-engine` harness and HUD Tauri bridge in `commands/workbench.rs` | React intent → Tauri approval resolver → `POST /v1/projects/validate` and `/v1/projects/attach` | **Converged implementation path:** React submits only `approvalReference`; Rust requires configured operator identity and an externally issued Oromë envelope | Rust refuses to mint approval. It rejects absent configuration, wrong schema/reference/decision, incomplete lineage, future timestamps, and expired envelopes. It derives stable resource/action idempotency and forwards only the resolved envelope. | Focused Rust tests cover valid, expired, mismatched, denied, and browser-envelope rejection; engine project tests prove typed attachment, replay, and unknown-field rejection. A native configured-envelope receipt remains outstanding acceptance evidence. |
| Workbench objective and run planning | `arda-engine` durable run store through HUD Rust `canonical_run_graph` | React objective intent → Tauri → `POST /v1/runs/plan` | **Converged implementation path:** Rust creates stable run/objective IDs and the six-node graph; engine validates it, replaces project digest, and persists events/checkpoint | React no longer exports `buildRunGraph` and cannot submit IDs, nodes, edges, budgets, authority, retries, checkpoints, provenance, approval decisions, or idempotency keys. | Rust tests prove stable graph generation and reject unknown browser graph fields; engine run tests prove plan replay and durable recovery. |
| Workbench approval, cancellation, provider execution, operator completion, and review | `arda-engine` harness run endpoints | React intent/reference → Tauri resolved envelope → typed HTTP mutations | **Converged implementation path:** UI state changes only from returned durable `RunRecord`; engine validates receipt/evidence and owns transitions | Approval decisions, timestamps, proposal IDs, and idempotency keys are absent from browser payloads. The existing engine mutation/review guards and durable journal remain authoritative. | Engine harness tests pass approve, reject/cancel, provider/receipt completion, failed verification, retry/idempotency, restart, and replay paths. Native configured-envelope walkthrough remains outstanding. |
...
| Research questions, watchlists, briefs, pause/resume/retire | `arda-engine::harness::research_operator` behind HUD Tauri `commands/research.rs` | React intent → Tauri approval resolver → typed harness HTTP | **Converged implementation path:** Rust owns question/watchlist IDs, operator identity, expiry, approval resolution, timestamps, and stable idempotency; one versioned aggregate projection reports source revision/time, partial failures, stale state, and recovery | Browser payloads contain only bounded domain intent plus an approval reference. Engine registries remain versioned, atomic, durable, and covered by restart recovery. | Rust authority tests, engine registry recovery tests, frontend Tauri projection tests, and focused/full HUD gates. |
| Personal Operations snapshot and capture/classification/reminder/data controls | `arda-engine::harness::personal_ops` and append-only personal store behind HUD Tauri `commands/personal_ops.rs` | React intent → installed Rust authority → typed harness HTTP | **Converged implementation path:** Rust requires configured operator identity, owns stable operator-scoped mutation idempotency and evidence metadata, and derives one versioned `arda.hud.personal-ops-projection.v1` snapshot from one durable engine projection revision with source time, stale state, and recovery action | Browser payloads contain only capture/classification/reminder/destructive-confirmation intent. No browser default operator, direct loopback HTTP, timestamp/random retry key, evidence class, rationale, or receipt authority remains. Engine export/delete remain operator-scoped and preserve system receipts. | Six HUD Rust authority/projection tests, four frontend Tauri-boundary tests, five component tests, eleven engine harness tests, and five append-only store tests cover identity isolation, exact-once replay, restart-backed projection, export/deletion, and receipt preservation. Native configured-identity walkthrough remains acceptance evidence. |
