...
| Converge frontend/backend one vertical workflow at a time with Rust authority and restart recovery | **active / blocked** | Monitor-session authority and durable restart recovery pass. The Manwë half of system/runtime health now uses one versioned Rust projection and preserves partial source truth, with four Rust aggregation tests, 509 HUD tests, `cargo check`, and production build passing. Hermes health remains the next C1.1 blocker; Workbench, Research, and Personal Operations follow serially. |
...
| Leave one live authority per domain | **blocked** | The monitor-session registry has durable Rust authority with atomic five-session restart recovery, and installed Manwë health now has one Rust aggregation authority. Hermes health, Workbench, Research, and Personal Operations retain open convergence work. |
