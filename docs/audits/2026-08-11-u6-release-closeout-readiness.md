...
| Converge frontend/backend one vertical workflow at a time with Rust authority and restart recovery | **active / blocked** | Monitor-session authority/restart recovery and C1.1 system health now pass their implementation gates. Manwë and Hermes each have a versioned Rust projection; seven focused Rust tests, 510 HUD tests, `cargo check`, and production build pass. Workbench, Research, and Personal Operations remain, in that order. |
...
| Leave one live authority per domain | **blocked** | Monitor sessions and both system-health domains now have one installed Rust authority. Workbench, Research, and Personal Operations retain open convergence work. |
