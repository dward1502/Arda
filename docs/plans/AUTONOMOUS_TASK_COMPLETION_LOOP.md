...
- Evidence boundary: this is queue-level source/package proof, not complete T7 acceptance. The installed executor still serializes through its existing root lock; durable checkpoint preservation, last-eligible-node resume, bounded read-only parallelism, real separate-project execution, dirty-worktree isolation, process restart, and installed-runtime proof remain open.
