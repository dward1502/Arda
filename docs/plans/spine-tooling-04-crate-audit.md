...
### arda-mandos  [1493 LoC, 3 rev-deps, path-deps 3]
...
- What it does: reasoning — context, reasoning, scoring, pageindex, notify.
- Evidence: 7 pub mods. MANDOS target.
- Current state: `arda-mandos` crate already exposes `OracleService` and
  `OracleDaemon/OracleDaemonConfig`; `cli` commands still use
  `oracle_call_or_local()` and the `start` branch still launches
  `OracleDaemon`. `prometheus` currently uses apollo only.
- Disposition: **KEEP as lib for now; tighten audit to lib-only**.
  ...
  `OracleService::*` calls. Do not attempt transport removal until
...
### arda-onboarding  [3244 LoC, 1 rev-dep (cli)]
...
| arda-mnemosyne | 3143 | 8 | SHRINK→lib | vaire |
| arda-plutus | 2255 | 11 | SHRINK→lib | arda-economics |
...
| arda-apollo | 2179 | 2 | MERGE | → aulë ||
| arda-hades | 8177 | 2 | **DELETE** (remake later) | — ||
| arda-warden | 1541 | 3 | **DELETE** | — ||
...
## Verified workspace state (from cargo metadata / root Cargo.toml)

Current workspace members (as of `crates` tree and root `Cargo.toml`):
`arda`, `arda-athena`, `arda-aule`, `arda-charon`, `arda-core`, `arda-engine`,
`arda-fleet`, `arda-forge-mind`, `arda-governance`, `arda-human`, `arda-mandos`,
`arda-orome`, `arda-plutus`, `arda-prometheus`, `arda-vaire`, `manwe`.

Verified discrepancies vs audit checklist:
- `arda-governance`: checklist collapses it into folded state, but it is still
  listed in root `Cargo.toml` members and visible in `cargo metadata`.
- `arda-plutus` / `arda-vaire` / `arda-mandos`: checklist splits them across
  folded/Silmarillion outcomes; all three remain literal workspace crates today.
- `arda-charon` / `arda-fleet`: live under `crates/old-annunimas/`; checklist says
  delete but they still build as workspace members.

### Blocker for next removal batch (`2026-07-15`)
 `arda-fleet` was removed after reverse-dep check:
 - `cargo tree --invert -p arda-fleet` showed 0 workspace reverse-deps.
 - `arda-prometheus` was assumed safe, but its manifest still declares:
     arda-fleet = { path = "../../../old-annunimas/arda-fleet" }
     `cargo metadata` now fails with missing manifest until this is fixed.
 - Same crate has live code using `arda_fleet::{...}` under `src/core_link/`,
   `src/pipeline/`, etc.
- Consequence: any additional deletions from this checklist batch are blocked
  until `arda-prometheus` is either stripped of fleet usage or itself removed.

 ## Execution order (per refactor plan)

- [x] arda-signal-grid — folded/disposition executed
- [x] arda-service-registry — folded into `arda-core`
- [x] arda-systemd — fold into `arda-core`
- [x] arda-council — merged into `arda-aule`
- [x] arda-tool-harness — fold into `arda-core`
- [ ] arda-human — fold into `arda-varda` 
- [x] arda-comm — folded into `arda-orome`
- [x] arda-mcp — merge into `orome`
- [x] arda-hermes — merged into `arda-orome`
- [x] arda-apollo — merge into `aulë`
- [ ] arda-athena — shrink into `arda-varda`
- [ ] arda-mandos — keep as lib `arda-mandos`
- [x] arda-chronos — delete
- [ ] arda-hades — delete / remake later
- [ ] arda-warden — delete
- [ ] arda-fleet — delete
- [ ] arda-forge-mind — delete
- [ ] arda-onboarding — port to launcher app
- [x] arda-cli — deleted
- [x] arda-ceo — delete
- [ ] arda-prometheus — shrink into `aulë` (last)
